                            ...
                            <Select value={driverFilter} onValueChange={handleDriverChange}>
                              ...
                              <SelectContent style={{ background: 'var(--bg-white)', borderColor: 'var(--border-slate-200)' }}>
                                <SelectItem value="all" style={{ color: 'var(--text-slate-900)' }}>All Drivers</SelectItem>
                                  {sortUsers((effectiveDrivers || []).filter((d) => userHasRole(d, 'driver'))).map((driver) => {const dup = (effectiveDrivers || []).filter((x) => getDriverDisplayName(x) === getDriverDisplayName(driver));const name = dup.length > 1 ? `${getDriverDisplayName(driver)} (${driver.id.slice(-4)})` : getDriverDisplayName(driver);return <SelectItem key={driver.id || driver.appUserId || getDriverDisplayName(driver)} value={driver.id} style={{ color: 'var(--text-slate-900)' }}>{name}</SelectItem>;})}
                              </SelectContent>
