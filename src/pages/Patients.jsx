...
import MobilePatientDetailsSheet from "../components/patients/MobilePatientDetailsSheet";
...
// Consolidated into geoUtils (Sep 6 2026) — km, no input guards (previous semantics).
const calculateDistance = (lat1, lon1, lat2, lon2) => haversineKm(lat1, lon1, lat2, lon2);
...
import StoreOverview from '../components/patients/StoreOverview';
...
export default function Patients() {
  ...
  // Device detection
  const { isMobile, deviceType: patientsDeviceType } = useDevice();
  // log on mount only
  React.useEffect(() => {
    console.log(`[Patients - isMobile] deviceType: ${patientsDeviceType}, isMobile: ${isMobile}`);
  ...
  useEffect(() => {
    ...
    return subscribePatientsRealtime({
      onPatientChange: (event) => {
        if (event.type === 'create') {
          setAllPatients((prev) => prev.some((patient) => patient?.id === event.id) ? prev : [...prev, event.data]);
        } else if (event.type === 'update') {
          setAllPatients((prev) => prev.map((patient) => patient?.id === event.id ? { ...patient, ...event.data } : patient));
          setSelectedPatient((prev) => prev?.id === event.id ? { ...prev, ...event.data } : prev);
        } else if (event.type === 'delete') {
          setAllPatients((prev) => prev.filter((patient) => patient?.id !== event.id));
          setSelectedPatient((prev) => prev?.id === event.id ? null : prev);
        }
      },
      onDeliveryChange: (event) => {
        if (event.type === 'create') {
          setDeliveries((prev) => prev.some((delivery) => delivery?.id === event.id) ? prev : [...prev, event.data]);
          setAllDriverDeliveries((prev) => prev.some((delivery) => delivery?.id === event.id) ? prev : [...prev, event.data]);
        } else if (event.type === 'update') {
          setDeliveries((prev) => prev.map((delivery) => delivery?.id === event.id ? { ...delivery, ...event.data } : delivery));
          setAllDriverDeliveries((prev) => prev.map((delivery) => delivery?.id === event.id ? { ...delivery, ...event.data } : delivery));
        } else if (event.type === 'delete') {
          setDeliveries((prev) => prev.filter((delivery) => delivery?.id !== event.id));
          setAllDriverDeliveries((prev) => prev.filter((delivery) => delivery?.id !== event.id));
        }
      }
    });
  }, [hasAccess]);
  ...
  // Show import button only for app owners / admins
  const showImportButton = isAppOwner(currentUser) || userHasRole(currentUser, 'admin');
      ...
      return (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6">
            <Card className="mt-4 bg-surface border-surface">
              <CardContent className="p-6 text-center text-soft">
    ...
    if (storeData.length === 0) {
      return (
        <div className="flex-1 flex flex-col overflow-hidden">
          ...
          <div className="flex-1 overflow-y-auto px-6 pb-6">
            <Card className="mt-4 bg-surface border-surface">
              <CardContent className="p-6 text-center text-soft">No stores found for this city.</CardContent>
            </Card>
          </div>
        </div>);
  ...
  if (isLoading) {
    return (
      <div className="h-screen flex flex-col">
        <div className="flex-shrink-0 p-6 bg-surface" style={{ borderBottom: '1px solid var(--border-slate-200)' }}>
  ...
  return (
    <div className="h-screen flex flex-col" style={{ background: 'var(--bg-slate-50)' }}>
      ...
      {!inOverviewContext &&
      <div className="flex-shrink-0 shadow-sm bg-surface" style={{ borderBottom: '1px solid var(--border-slate-200)' }}>
            ...
            <>
                  ...
                  <div className="flex gap-2 flex-wrap">
                    ...
                    <div className="space-y-1 flex-1">
                      <span className="text-xs font-medium text-body-2">City</span>
                      <Select
                    ...
                    disabled={!userHasRole(currentUser, 'admin') && currentUser.city_id}>
                        <SelectTrigger className="w-full h-9 text-body bg-surface" style={{ borderColor: 'var(--menu-border)' }}>
                          <SelectValue placeholder="City..." />
                        </SelectTrigger>
                        <SelectContent className="max-h-[300px] overflow-y-auto text-body bg-surface border-surface">
                ...
                <div className="space-y-1 flex-1">
                        <span className="text-xs font-medium text-body-2">Store</span>
                        <Select
                    value={storeFilter}
                    onValueChange={(value) => {
                    ...
                    {/* Status Filter */}
                    <div className="space-y-1 flex-1">
                      <span className="text-xs font-medium text-body-2">Status</span>
                ...
                <Button variant="ghost" size="icon" className="absolute right-2 top-1/2 transform -translate-y-1/2 h-7 w-7" onClick={() => setSearchTerm("")}>
                  ...
                  <Popover open={isCalendarOpen} onOpenChange={(open) => { setIsCalendarOpen(open); if (open) setCalendarMonth(selectedDate instanceof Date ? selectedDate : new Date()); }}>
                    <PopoverTrigger asChild>
                      <Button variant="outline" size="sm" className="h-10 px-3 gap-2 text-sm font-medium flex-shrink-0 text-body bg-surface border-surface">
                        <CalendarIcon className="w-4 h-4" />
                        {selectedDate ? format(selectedDate instanceof Date ? selectedDate : new Date(selectedDate), 'EEE MMM dd') : 'Pick date'}
                      </Button>
                    </PopoverTrigger>
                    <PopoverContent className="w-auto p-0 z-[1000] text-body bg-surface border-surface" align="start">
                      <Calendar
                        mode="single"
                        selected={selectedDate instanceof Date ? selectedDate : new Date(selectedDate)}
                        onSelect={(date) => { if (!date) return; setIsCalendarOpen(false); }}
                        month={calendarMonth}
                  ...
                  <div className="flex gap-2">
                    ...
                    <div className="space-y-1 w-40 flex-shrink-0">
                      <span className="text-sm font-medium text-body-2">City</span>
                      <Select
                    value={selectedCityId}
                    onValueChange={(cityId) => {
                      setSelectedCityId(cityId);
                    ...
                    disabled={!userHasRole(currentUser, 'admin') && currentUser.city_id}>
                        <SelectTrigger className="w-full text-body bg-surface" style={{ borderColor: 'var(--menu-border)' }}>
                    ...
                    {!(userHasRole(currentUser, 'dispatcher') && !userHasRole(currentUser, 'admin') && currentUser.store_ids?.length === 1) &&
                <div className="space-y-1 w-40 flex-shrink-0">
                        <span className="text-sm font-medium text-body-2">Store</span>
                        <Select
                    value={storeFilter}
                    onValueChange={(value) => {
                      console.log(`[Patients] Store filter changed to: ${value}`);
                    ...
                    {/* Status Filter */}
                    <div className="space-y-1 w-40 flex-shrink-0">
                      <span className="text-sm font-medium text-body-2">Status</span>
                      <Select value={statusFilter} onValueChange={setStatusFilter}>
                        <SelectTrigger className="w-full text-body bg-surface" style={{ borderColor: 'var(--menu-border)' }}>
                          <SelectValue placeholder="Status..." />
                        </SelectTrigger>
      ...
      <div className="flex flex-1 overflow-hidden">
        ...
        <>
            ...
            <div
            className="flex flex-col overflow-hidden hidden lg:flex"
            style={{ width: `${rightPanelWidth}px`, flexShrink: 0, background: 'var(--bg-slate-100)', borderLeft: '1px solid var(--border-slate-200)' }}>
              ...
              onEditDelivery={(delivery) => {
                ...
                setPatientForNewDelivery(selectedPatient);
                setShowDeliveryForm(true);
              }}
              allPatients={allPatients}
              stores={stores}
      ...
      <AnimatePresence>
        ...
        <>
          ...
          <PatientForm
            ...
            onCreateDuplicate={(p) => {setPatientFormMode('newAddress');setEditingPatient(p);}}
      ...
      <AnimatePresence>
        ...
        <DeliveryForm
          ...
          onSave={(deliveryData) => {
            ...
            if (!editingDeliveryId) handleSaveDelivery(deliveryData);else
            {setShowDeliveryForm(false);setPatientForNewDelivery(null);}
          }}
          onCancel={() => {
            setEditingDeliveryId(null);
            setShowDeliveryForm(false);
          ...
          openMode={editingDeliveryId ? null : "add_to_route"} />
      ...
      {/* Mobile-only: slide-up patient details sheet when a patient is selected */}
      {isMobile &&
