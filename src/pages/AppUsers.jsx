...
export default function AppUsers() {
  ...
  // Real-time WebSocket subscription for AppUser entity changes
  useEffect(() => {
    const unsubscribe = base44.entities.AppUser.subscribe((event) => {
      if (!event) return;
      if (event.type === 'delete') {
        setAppUsers(prev => prev.filter(u => u.id !== event.id));
      } else if (event.data) {
        setAppUsers(prev => {
          const exists = prev.some(u => u.id === event.data.id);
          if (exists) return prev.map(u => u.id === event.data.id ? { ...u, ...event.data } : u);
  ...
  // Subscribe to live location/heartbeat broadcasts from the layout's smart refresh
  useEffect(() => {
    ...
    return () => window.removeEventListener('driverLocationsUpdated', handleDriverLocationsUpdated);
  }, []);
  ...
  const getUserStatusIndicator = (user) => {
    ...
    if (status === 'online' || status === 'on_duty') {
      if (!user.location_updated_at) return '#cbd5e1'; // gray (no heartbeat ever)
      const diffMinutes = (Date.now() - new Date(user.location_updated_at).getTime()) / 60000;
      if (diffMinutes < 5) return '#10b981';  // green  (active < 5 min)
      if (diffMinutes < 30) return '#f97316'; // orange (inactive 5-30 min)
      return '#cbd5e1';                        // gray   (offline > 30 min)
    }

    return '#cbd5e1'; // gray (off_duty)
  };
