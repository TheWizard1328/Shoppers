...
export default function Register() {
  ...
  const handleAcceptInvite = async () => {
    if (!currentUser) {
      ...
      base44.auth.redirectToLogin(`${window.location.origin}${window.location.pathname}${window.location.search}${window.location.hash}`);
      return;
    }
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 dark:bg-slate-800 p-4">
      <Card className="w-full max-w-md shadow-lg">
        ...
        <CardContent className="text-center space-y-4 pt-2">
          ...
          {status === 'already_used' && (
            <div className="py-4">
              <CheckCircle className="w-10 h-10 text-slate-400 dark:text-slate-400 mx-auto mb-3" />
