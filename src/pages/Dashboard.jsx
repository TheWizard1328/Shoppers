      ...
      <div className={statsCardPositioning} style={{ zIndex: 600 }}>
        ...
        onMouseLeave={() => handleStatsPanelInteraction(false)}>
          ...
          {(() => {
            ...
            const legendData=isAdmin?driversList.filter(driver=>deliveries.some(d=>d&&d.delivery_date===dateKey&&d.driver_id===driver.id)).map(driver=>{const s=(appUsers||[]).find(au=>au&&au.user_id===driver.id)?.driver_status;const c=s==='on_duty'?'#16a34a':s==='on_break'?'#3b82f6':s==='off_duty'?'#dc2626':'#94a3b8';return{driverId:driver.id,driverName:driver.user_name||driver.full_name||'Unknown',color:getDriverColor(driver),totalStops:deliveries.filter(d=>d&&d.delivery_date===dateKey&&d.driver_id===driver.id&&['completed','failed','cancelled'].includes(d.status)).length,statusRingColor:c};}):isAllDriversMode?[...driverRoutes].sort((a,b)=>(a.driverName||'').localeCompare(b.driverName||'')):[];
            if (!legendData.length) return null;
            return <div className="rounded-lg backdrop-blur-sm shadow-lg border" style={{ background: 'var(--bg-white)', opacity: 0.95, borderColor: 'var(--border-slate-200)', width: cardWidth }} onMouseEnter={() => handleCardInteraction(true)} onMouseLeave={() => handleCardInteraction(false)}><div className="flex w-full flex-wrap gap-x-0.5 gap-y-0.5 items-center justify-center">{legendData.map((route) => <div key={route.driverId} className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full shadow-sm flex-shrink-0" style={{ backgroundColor: isAllDriversMode ? route.color : route.statusRingColor, border: isAllDriversMode ? `3px solid ${route.statusRingColor || '#ffffff'}` : '0 solid transparent' }} /><span className="text-xs font-medium whitespace-nowrap" style={{ color: 'var(--text-slate-700)' }}>{route.driverName || 'Unknown'}</span><span className="text-xs" style={{ color: 'var(--text-slate-500)' }}>({route.totalStops})</span></div>)}</div></div>;
          })()}
        </div>
      </div>
