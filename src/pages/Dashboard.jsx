...
function Dashboard() {
  ...
  // CRITICAL: Re-process location data whenever appUsers OR deliveries changes (smart refresh updates)
  useEffect(() => {
    if (!isDataLoaded || !currentUser || !deliveries || !drivers || !appUsers) {
      return;
    }
    const appUsersData = appUsers || users?.filter((u) => u.user_id) || [];
    console.log(`🔄 [Dashboard] Re-processing location data - appUsers: ${appUsersData.length}, deliveries: ${deliveries.length}`);
    // CRITICAL: Force notify when deliveries change (forceNotify=true)
    driverLocationPoller.processLocationData(currentUser, deliveries, drivers, stores, appUsersData, selectedDate, true);
  }, [appUsers, deliveries, isDataLoaded, currentUser, drivers, stores, users, selectedDate]); // Fetch and display current-to-next polyline for display
