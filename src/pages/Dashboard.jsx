      ...
      <div className={statsCardPositioning} style={{ zIndex: 600 }}>
        ...
        onMouseLeave={() => handleStatsPanelInteraction(false)}>
          ...
          {(() => {
            ...
            return <div className="rounded-lg backdrop-blur-sm shadow-lg border" style={{ background: 'var(--bg-white)', opacity: 0.95, borderColor: 'var(--border-slate-200)' }} onMouseEnter={() => handleCardInteraction(true)} onMouseLeave={() => handleCardInteraction(false)}><div className="flex flex-wrap gap-x-0.5 gap-y-0.5 items-center justify-center">{legendData.map((route) => <div key={route.driverId} className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full shadow-sm flex-shrink-0" style={{ backgroundColor: isAllDriversMode ? route.color : route.statusRingColor, border: isAllDriversMode ? `3px solid ${route.statusRingColor || '#ffffff'}` : '0 solid transparent' }} /><span className="text-xs font-medium whitespace-nowrap" style={{ color: 'var(--text-slate-700)' }}>{route.driverName || 'Unknown'}</span><span className="text-xs" style={{ color: 'var(--text-slate-500)' }}>({route.totalStops})</span></div>)}</div></div>;
          })()}
        </div>
      </div>
