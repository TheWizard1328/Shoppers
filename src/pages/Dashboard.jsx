...
function Dashboard() {
  ...
  // CRITICAL: Re-process location data whenever appUsers changes (smart refresh updates)
  useEffect(() => {
    if (!isDataLoaded || !currentUser || !deliveries || !drivers || !appUsers) {
      return;
    }
    const appUsersData = appUsers || users?.filter((u) => u.user_id) || [];
    console.log(`🔄 [Dashboard] Re-processing location data - appUsers count: ${appUsersData.length}`);
    driverLocationPoller.processLocationData(currentUser, deliveries, drivers, stores, appUsersData, selectedDate);
  }, [appUsers]); // Fetch and display current-to-next polyline for display
