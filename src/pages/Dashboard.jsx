...
function Dashboard() {
  ...
  const stats = useMemo(() => {
    ...
    const completed=sd.filter((d)=>d&&d.status==='completed'&&!isRtn(d)).length+rd.filter((d)=>d&&!d.patient_id&&d.after_hours_pickup&&(d.status==='completed'||d.status==='cancelled')).length;
    const returned=sd.filter(isRtn).length; const failed=sd.filter((d)=>d&&((d.status==='failed'&&!isRtn(d))||(d.status==='cancelled'&&!d.patient_id))).length;
    const completedPickups=ap.filter((d)=>d&&(d.status==='completed'||d.status==='cancelled')).length;
    const totalPickups=ap.length; let totalDrivers=0,inTransitDrivers=0,completedDrivers=0;
    ...
    return {total,inTransit,enRoute,completed,failed,returned,totalDrivers,inTransitDrivers,completedDrivers,totalPickups,completedPickups};
  ...
  const driversList = useMemo(() => { const _ds=format(selectedDate,'yyyy-MM-dd'),_iok=isAdmin?new Set((deliveries||[]).filter((d)=>d&&d.delivery_date===_ds&&d.driver_id).map((d)=>d.driver_id)):new Set(); const src=(appUsers||[]).filter((au)=>au&&au.user_id&&au.app_roles?.includes('driver')&&(au.status==='active'||_iok.has(au.user_id))).map((au)=>({...au,id:au.user_id})).sort((a,b)=>{const sa=a.sort_order??Infinity,sb=b.sort_order??Infinity;return sa!==sb?sa-sb:(a.user_name||'').toLowerCase().localeCompare((b.user_name||'').toLowerCase());}); if(userHasRole(currentUser,'admin')||userHasRole(currentUser,'driver'))return src; if(userHasRole(currentUser,'dispatcher')){const sid=String((currentUser.store_ids||[])[0]||'');const st=stores?.find((s)=>s&&String(s.id)===sid);const aids=new Set();if(st){const di=new Date(selectedDate).getDay();if(di===6){if(st.saturday_am_driver_id)aids.add(st.saturday_am_driver_id);if(st.saturday_pm_driver_id)aids.add(st.saturday_pm_driver_id);}else if(di===0){if(st.sunday_am_driver_id)aids.add(st.sunday_am_driver_id);if(st.sunday_pm_driver_id)aids.add(st.sunday_pm_driver_id);}else{if(st.weekday_am_driver_id)aids.add(st.weekday_am_driver_id);if(st.weekday_pm_driver_id)aids.add(st.weekday_pm_driver_id);}}const sds=format(selectedDate,'yyyy-MM-dd');(deliveries||[]).filter((d)=>d&&d.delivery_date===sds&&String(d.store_id)===sid).forEach((d)=>{if(d.driver_id)aids.add(d.driver_id);});(window.__dispatcherOverrideDriverIds?.[sds]||[]).forEach((id)=>aids.add(id));return src.filter((d)=>aids.has(d.id)||aids.has(d.user_id));}return src;}, [appUsers, currentUser, stores, selectedDate, deliveries, isAdmin]);
  ...
  const nextStopCoordinates = useMemo(() => { if (!nextStop) return null; if (nextStop.patient_id) { const p = patients.find((p) => p && p.id === nextStop.patient_id); if (p?.latitude && p?.longitude) return { lat: p.latitude, lon: p.longitude }; } else if (isInterStoreDelivery(nextStop.delivery_id)) { const isl = getInterStoreLocationSync(nextStop.delivery_id); if (isl?.store_latitude && isl?.store_longitude) return { lat: isl.store_latitude, lon: isl.store_longitude }; const s = stores.find((s) => s && s.id === nextStop.store_id); if (s?.latitude && s?.longitude) return { lat: s.latitude, lon: s.longitude }; } else if (nextStop.store_id) { const s = stores.find((s) => s && s.id === nextStop.store_id); if (s?.latitude && s?.longitude) return { lat: s.latitude, lon: s.longitude }; } return null; }, [nextStop, patients, stores]);
...
useEffect(() => {
  ...
  const handleDateChange = async (date) => {
    ...
    try {
      ...
      // Dispatcher: schedule override → store default (1 driver) → deliveries → 'all' if multiple
      if (isDispatcher && !isAdmin) { const _dayIdx=new Date(dateStr+'T00:00:00').getDay(),_isSat=_dayIdx===6,_isSun=_dayIdx===0;const _storeId=String((currentUser?.store_ids||[])[0]||'');const _store=(stores||[]).find((s)=>s&&String(s.id)===_storeId);const _driverInAU=(id)=>!!id&&appUsers.some((au)=>au?.user_id===id&&au?.app_roles?.includes('driver'));const _def=()=>{if(!_store)return'';if(_isSat)return _store.saturday_am_driver_id||_store.saturday_pm_driver_id||'';if(_isSun)return _store.sunday_am_driver_id||_store.sunday_pm_driver_id||'';return _store.weekday_am_driver_id||_store.weekday_pm_driver_id||'';};let _f='';if(_storeId){try{const _ov=await base44.entities.DriverScheduleOverride.filter({date:dateStr,store_id:_storeId});const _ovId=(_ov||[]).find((o)=>o&&String(o.store_id)===_storeId)?.driver_id||'';if(!window.__dispatcherOverrideDriverIds)window.__dispatcherOverrideDriverIds={};window.__dispatcherOverrideDriverIds[dateStr]=_ovId?[_ovId]:[];_f=_ovId&&_driverInAU(_ovId)?_ovId:'';if(!_f)_f=_def();}catch(_){_f=_def();}}setSelectedDriverId(_f);globalFilters.setSelectedDriverId(_f); }
