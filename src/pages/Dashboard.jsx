...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    // Reduced from 600ms to 100ms: the stats card uses transform (doesn't affect
    // offsetHeight) and the stop cards use height: 0→undefined (instant, not a CSS
    // transition). The 600ms delay was causing a "double zoom" — the first animation
    // (from a GPS tick just before the immersive transition) would use immersive
    // padding, then the refit at 600ms would queue behind it and fire a second
    // animation with non-immersive padding. With cancelInFlight, the refit cancels
    // any in-flight animation and immediately starts the correct one.
    const delay = isExitingImmersive ? 100 : 350;
    const t = setTimeout(() => {
      lastProgrammaticMapMoveRef.current = Date.now();
      window._lastProgrammaticMapMove = Date.now();
      // Signal that the next shouldFitBounds should cancel any in-flight animation
  ...
  const handleMapViewCycle = useCallback(() => {
    ...
    const goPhase = (nextPhase, shouldLock, unlockMs = null) => {
      ...
      window._userMapControlUntil = 0;
      if (nextPhase === 1) { lastUserInteractionRef.current = Date.now(); }
      ...
      window._cancelInFlightNextFit = true;
      // Unified suppression lock after a FAB tap: useDriverLocationSync GPS/WS/watchdog
  ...
  useEffect(() => {
    ...
    // Check if this trigger should cancel any in-flight map animation
    // (set by the immersive-mode refit effect to avoid double-zoom)
    const _cancelInFlight = window._cancelInFlightNextFit === true;
    if (_cancelInFlight) window._cancelInFlightNextFit = false;
...
// Fire FAB as soon as markers are rendered — no need to wait for route lines/locations
  ...
  useEffect(() => {
    ...
    const preRenderSync = async () => {
      try {
        ...
        const freshAppUsers = await queueEntityRequest(() => base44.entities.AppUser.list(null, null, null, 'id,user_id,user_name,app_roles,status,driver_status,driver_id,driver_name,store_ids,city_id,city_ids,home_latitude,home_longitude,current_latitude,current_longitude,location_tracking_enabled,location_updated_at,preferred_travel_mode,sort_order,role,full_name,created_date,updated_date'), 'Dashboard STEP 0 AppUser list');
  ...
  useEffect(() => {
    ...
    const backgroundPrioritySync = async () => {
      try {
        ...
        if (isTodaySelected && (!t || Date.now() - t >= (active ? 180000 : 600000))) setTimeout(() => window.dispatchEvent(new CustomEvent('triggerPullToSync', { detail: { silent: true, reason: active ? 'initial_load_today_active_routes' : 'initial_load_today_completed_routes' } })), 2000);
      } catch (error) {
  ...
  const dashboardProps = {
    ...
    refreshData, refreshUser, updateDeliveriesLocally, updateAppUsersLocally, dataSource,
  };
