      ...
      case 3: // "Center on Incomplete Stops Only + Their Drivers + Pending"
        ...
        if (isDispatcher && !isAdmin) {
          ...
          if (allCoordinatesPhase3.length > 0) {
            ...
            const maxZoomD = Math.max(12.0, Math.min(19, Math.round((16 - Math.log2(spanKmD + 1) * 1.2) * 10) / 10));
            window._lastProgrammaticMapMove = Date.now();
            setShouldFitBounds({
      ...
      <div className={statsCardPositioning} style={{ zIndex: 600 }}>
        ...
        onMouseLeave={() => handleStatsPanelInteraction(false)}>
          ...
          {(() => {
            ...
            const legendData = isAdmin ? driversList.filter((driver) => deliveries.some((d) => d && d.delivery_date === dateKey && d.driver_id === driver.id)).map((driver) => ({ driverId: driver.id, driverName: driver.user_name || driver.full_name || 'Unknown', color: getDriverColor(driver), totalStops: deliveries.filter((d) => d && d.delivery_date === dateKey && d.driver_id === driver.id).length, statusRingColor: ((appUsers || []).find((au) => au && au.user_id === driver.id)?.driver_status === 'on_duty' ? '#16a34a' : (appUsers || []).find((au) => au && au.user_id === driver.id)?.driver_status === 'on_break' ? '#f97316' : (appUsers || []).find((au) => au && au.user_id === driver.id)?.driver_status === 'off_duty' ? '#dc2626' : '#ffffff') })) : isAllDriversMode ? [...driverRoutes].sort((a, b) => (a.driverName || '').localeCompare(b.driverName || '')) : [];
            if (!legendData.length) return null;
            return <div className="rounded-lg backdrop-blur-sm shadow-lg border" style={{ background: 'var(--bg-white)', opacity: 0.95, borderColor: 'var(--border-slate-200)' }} onMouseEnter={() => handleCardInteraction(true)} onMouseLeave={() => handleCardInteraction(false)}><div className="flex flex-wrap gap-x-0.5 gap-y-0.5 items-center justify-center">{legendData.map((route) => <div key={route.driverId} className="flex items-center gap-1.5"><div className="w-3 h-3 rounded-full border-[3px] shadow-sm flex-shrink-0" style={{ backgroundColor: route.color, borderColor: route.statusRingColor || '#ffffff' }} /><span className="text-xs font-medium whitespace-nowrap" style={{ color: 'var(--text-slate-700)' }}>{route.driverName || 'Unknown'}</span><span className="text-xs" style={{ color: 'var(--text-slate-500)' }}>({route.totalStops})</span></div>)}</div></div>;
          })()}
        </div>
      </div>
