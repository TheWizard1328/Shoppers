      ...
      <div className={statsCardPositioning} style={{ zIndex: 600 }}>
        ...
        onMouseLeave={() => handleStatsPanelInteraction(false)}>
          ...
          {(() => {
            ...
            const legendSourceDeliveries=(deliveries||[]).filter(d=>d&&d.delivery_date===dateKey&&d.driver_id); const legendData=Array.from(new Set(legendSourceDeliveries.map(d=>d.driver_id))).map(driverId=>{const driver=driversList.find(d=>d&&d.id===driverId)||appUsers.find(au=>au&&au.user_id===driverId);const s=(appUsers||[]).find(au=>au&&au.user_id===driverId)?.driver_status;const c=s==='on_duty'?'#16a34a':s==='on_break'?'#3b82f6':s==='off_duty'?'#dc2626':'#94a3b8';return{driverId,driverName:driver?.user_name||driver?.full_name||'Unknown',sortOrder:driver?.sort_order??Infinity,color:getDriverColor(driver||{id:driverId}),totalStops:legendSourceDeliveries.filter(d=>d&&d.driver_id===driverId&&d.patient_id&&String(d.patient_id).trim()!=='').length+legendSourceDeliveries.filter(d=>d&&d.driver_id===driverId&&(!d.patient_id||String(d.patient_id).trim()==='')&&d.after_hours_pickup===true).length,statusRingColor:c,isAllDriversMode:isAllDriversMode};}).sort((a,b)=>(a.sortOrder??Infinity)!==(b.sortOrder??Infinity)?(a.sortOrder??Infinity)-(b.sortOrder??Infinity):(a.driverName||'').localeCompare(b.driverName||''));
