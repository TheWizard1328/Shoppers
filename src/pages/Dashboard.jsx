...
function Dashboard() {
  ...
  const handleDateChange = async (date) => {
    ...
    try {
      ...
      centerNextDeliveryCard(deliveriesWithStopOrder);
      fabControlEvents.notifyDataReady();
      if (isDispatcher && !isAdmin) { const _si=(currentUser?.store_ids||[]).map(String),_dws=[...new Set(priorityDeliveries.filter((d)=>d&&_si.includes(String(d.store_id))).map((d)=>d.driver_id).filter(Boolean))]; const _di=new Date(dateStr+'T00:00:00').getDay(),_ids=new Set();(stores||[]).filter((s)=>s&&_si.includes(String(s.id))).forEach((s)=>{const sat=_di===6,sun=_di===0;if(sat){if(s.saturday_am_driver_id)_ids.add(s.saturday_am_driver_id);if(s.saturday_pm_driver_id)_ids.add(s.saturday_pm_driver_id);}else if(sun){if(s.sunday_am_driver_id)_ids.add(s.sunday_am_driver_id);if(s.sunday_pm_driver_id)_ids.add(s.sunday_pm_driver_id);}else{if(s.weekday_am_driver_id)_ids.add(s.weekday_am_driver_id);if(s.weekday_pm_driver_id)_ids.add(s.weekday_pm_driver_id);}}); let _aid=null; if(_dws.length===1){_aid=_dws[0];}else if(_dws.length>1){const _m=_dws.filter((id)=>_ids.has(id));_aid=_m.length===1?_m[0]:'all';}else{const _da=[..._ids];_aid=_da.length===1?_da[0]:null;} const _final = _aid || ''; setSelectedDriverId(_final); globalFilters.setSelectedDriverId(_final); }
