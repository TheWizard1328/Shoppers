...
function Dashboard() {
  ...
  useEffect(() => {
    ...
    console.log('🔌 [Dashboard] Setting up Delivery subscription with debouncing...');
    const unsubscribeDeliveries = base44.entities.Delivery.subscribe((event) => {
      ...
      // Schedule update for debounced batch processing
      if (event.type === 'create' && event.data) {
        scheduleDeliveryUpdate('create', event.data);
      } else if (event.type === 'update' && event.data) {
        scheduleDeliveryUpdate('update', event.data);
      } else if (event.type === 'delete' && event.id) {
        scheduleDeliveryUpdate('delete', event.id);
      }
    });
  ...
  useEffect(() => { if (!currentUser?.id||!isDataLoaded) { setPerformanceStats(null); setIsLoadingPayrollStats(false); return; } const dId=(selectedDriverId&&selectedDriverId!=='all')?selectedDriverId:(isDriver?currentUser?.id:null); if(!dId){setPerformanceStats(null);setIsLoadingPayrollStats(false);return;} let c=false; const dateStr=format(selectedDate,'yyyy-MM-dd'); const f=async()=>{setIsLoadingPayrollStats(true);console.log(`💰 [PayStats] Fetching driver=${dId} date=${dateStr}`);try{const mod=await import('@/functions/getDeliveryStats');const getDeliveryStatsFn=mod.getDeliveryStats||mod.default;const r=await getDeliveryStatsFn({selectedDate:dateStr,driverId:dId});const d=r?.data||r;console.log('💰 [PayStats] Result:',JSON.stringify(d?.performanceStats));if(!c){if(d?.performanceStats)setPerformanceStats(d.performanceStats);else setPerformanceStats(null);}}catch(e){console.error('❌ [PayStats] Error:',e);if(!c)setPerformanceStats(null);}finally{if(!c)setIsLoadingPayrollStats(false);}};f();const h=()=>{if(!c)f();};window.addEventListener('refreshDeliveryStats',h);return()=>{c=true;window.removeEventListener('refreshDeliveryStats',h);};
