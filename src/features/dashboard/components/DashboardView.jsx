...
export default function DashboardView({
  ...
  setIsEntityUpdating, isEntityUpdating, hasRateLimitError, updateDeliveriesLocally, updateAppUsersLocally,
  // Cards
  ...
  // Stamp markerClickedAtRef whenever the map dispatches a centerStopCard event
  // (fired directly from handleMarkerClickForFanning, before handleMarkerClick prop is called)
  useEffect(() => {
    const handleCenterStopCard = () => { markerClickedAtRef.current = Date.now(); };
    window.addEventListener('centerStopCard', handleCenterStopCard);
    return () => window.removeEventListener('centerStopCard', handleCenterStopCard);
  }, []);
  ...
  useEffect(() => {
    ...
    // Wrap handleMarkerClick to stamp markerClickedAtRef so handleCenteredCardChange skips its fitBounds
    const wrappedHandleMarkerClick = (delivery, markerType) => {
      markerClickedAtRef.current = Date.now();
      handleMarkerClick?.(delivery, markerType);
    };

    // Handles a card being centered after a manual swipe.
    // Unlocks the FAB and max-zoom fits the map to the driver + card marker.
    const handleCenteredCardChange = ({ deliveryId, driverId } = {}) => {
    // Skip if triggered within 3s of a marker click — the map already panned to the marker
    if (Date.now() - markerClickedAtRef.current < 3000) return;
    if (!deliveryId || !driverId) return;
    const delivery = deliveriesWithStopOrder.find(d => d?.id === deliveryId);
    if (!delivery) return;

    // Resolve marker coords
    ...
    if (driverLocation?.latitude && driverLocation?.longitude) {
      driverLat = driverLocation.latitude; driverLng = driverLocation.longitude;
    } else {
      const targetDriverId = selectedDriverId && selectedDriverId !== 'all' ? selectedDriverId : driverId;
      const driverAppUser = appUsers.find(u => u?.user_id === targetDriverId || u?.id === targetDriverId);
      if (driverAppUser?.current_latitude && driverAppUser?.current_longitude) {
        driverLat = driverAppUser.current_latitude; driverLng = driverAppUser.current_longitude;
      } else {
        ...
        if (locEntry?.latitude && locEntry?.longitude) {
          driverLat = locEntry.latitude; driverLng = locEntry.longitude;
        }
      }
    }

    if (!cardLat || !cardLng) return;
    ...
    setShouldFitBounds({
      bounds,
      options: { ...(padding || {}), maxZoom: 17.5, animate: true, duration: 0.35 }
    });
    ...
    <div className="h-full w-full flex flex-col overflow-hidden" style={{ background: 'var(--bg-slate-50)' }}>
      ...
      <div className="flex-1 w-full relative min-h-0 overflow-hidden">
        ...
        <DashboardMapSection
          ...
          stopCardsBaseHeight={stopCardsBaseHeight} handleMarkerClick={wrappedHandleMarkerClick}
        ...
        <div
          className="absolute inset-x-0 bottom-0 z-[260]"
          style={{
