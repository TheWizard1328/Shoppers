    ...
    <div className="px-4 py-2 space-y-1 border-t" style={{ background: 'var(--bg-white)', borderColor: 'var(--border-slate-200)' }}>
      {drivers.map((driver) => (
        <div key={driver.name} className="flex items-center gap-2 text-xs">
          ...
          />
          <span className="font-medium" style={{ minWidth: '80px', color: 'var(--text-slate-700)' }}>
            {driver.name}
          </span>
          <span>
            <span className={driver.latChanged ? 'text-green-500' : ''} style={!driver.latChanged ? { color: 'var(--text-slate-600)' } : {}}>
              {driver.lat?.toFixed(6) || '?'}
            </span>
            {', '}
            <span className={driver.lngChanged ? 'text-green-500' : ''} style={!driver.lngChanged ? { color: 'var(--text-slate-600)' } : {}}>
              {driver.lng?.toFixed(6) || '?'}
            </span>
          </span>
          <span className={driver.timestampChanged ? 'text-green-500' : ''} style={driver.timestampChanged ? { marginLeft: 'auto' } : { marginLeft: 'auto', color: 'var(--text-slate-500)' }}>
