  ...
  const driverRoutes = useMemo(() => {
    ...
    const routes = Array.from(byDriver.values()).sort((a, b) => a.sortOrder - b.sortOrder).map((route) => {
      ...
      return {
        ...
        totalStops: stops.filter((stop) => stop.markerType === "delivery" && ["completed", "failed"].includes(stop.status)).length + stops.filter((stop) => stop.after_hours_pickup && ["completed", "failed"].includes(stop.status)).length,
