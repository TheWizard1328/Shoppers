  ...
  const driverRoutes = useMemo(() => {
    ...
    const routes = Array.from(byDriver.values()).sort((a, b) => a.sortOrder - b.sortOrder).map((route) => {
      ...
      return {
        ...
        totalStops: stops.filter((stop) => stop.markerType === "delivery" && FINISHED_STATUSES.includes(stop.status)).length + stops.filter((stop) => stop.after_hours_pickup && FINISHED_STATUSES.includes(stop.status)).length,
