...
export default function DeliveryMap({
  ...
  // CRITICAL: Track immersive exit so phase 2 follow doesn't reposition the map
  // immediately when the UI slides back in (e.g. after a double-tap to exit).
  const prevImmersiveHiddenRef = useRef(immersiveHidden);
  ...
  useEffect(() => {
    ...
    // CRITICAL: Don't reposition map immediately after exiting immersive mode —
    // the driver just double-tapped to show the UI; let them see where they are.
    if (immersiveExitSuppressUntilRef.current > Date.now()) return;

    const targetDriverId = selectedDriverId && selectedDriverId !== "all" ? selectedDriverId : currentUser?.id;
    if (!targetDriverId) return;

    // Only use the viewer's live GPS when THEY are the selected driver — never use the viewer's
    // location when an admin or dispatcher is watching a different driver's route.
    const _cdl = currentDriverLocationRef.current;
    const livePrimaryDriverMarker = _cdl?.latitude && _cdl?.longitude && targetDriverId === currentUser?.id
      ? {
          driverId: targetDriverId,
          driver_id: targetDriverId,
          latitude: _cdl.latitude,
          longitude: _cdl.longitude
        }
      : null;

    // For the selected driver, prefer: live GPS (self only) → shared location marker → appUser stored coords.
    // NEVER fall back to routeAwareCurrentDriverMarker when a different driver is selected —
    // that marker always belongs to the viewing user, not the selected driver.
    const selectedDriverSharedMarker = (routeAwareDriverLocationMarkersRef.current || []).find((marker) =>
      marker?.driverId === targetDriverId ||
      marker?.driver_id === targetDriverId ||
      marker?.user_id === targetDriverId ||
      marker?.id === targetDriverId
    );
    const isSelfSelected = targetDriverId === currentUser?.id;
    const targetDriverMarker = livePrimaryDriverMarker ||
      selectedDriverSharedMarker ||
      (isSelfSelected && routeAwareCurrentDriverMarkerRef.current ? routeAwareCurrentDriverMarkerRef.current : null);
    ...
    // Stamp BEFORE any zoom/pan operations so zoomend/moveend guards cover the full sequence
    window._lastProgrammaticMapMove = Date.now() + 1500; // extra lead for animated move
    ...
    if (alreadyAtMaxZoom) {
      ...
      map.fitBounds(
        ...
        ],
        {
          paddingTopLeft,
          paddingBottomRight,
          maxZoom: PHASE2_MAX_ZOOM,
          animate: true,
          duration: 0.5
        }
      );
    }
  // BLINK FIX: currentDriverLocation, routeAwareCurrentDriverMarker, and routeAwareDriverLocationMarkers
  // are intentionally read via refs inside this effect (not listed as deps).
  // GPS fires a new {lat,lng} object every ~1s via watchPosition(maximumAge:0), making
  // currentDriverLocation a new reference every tick. Putting it in deps caused React to
