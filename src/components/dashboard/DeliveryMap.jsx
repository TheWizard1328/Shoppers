        ...
        {pickupMarkers.map((pickup, index) => {
          ...
          return [
            ...
            <Marker
              ...
              {!pickup.useSimpleCircle && !pickup.isOtherDriver && (isClustered && !isFanned ? (<Popup autoPan={false} closeButton={false} offset={[0,-20]} className="custom-popup"><div className="min-w-[200px] max-w-[300px] space-y-2"><div className="font-semibold text-sm pb-1 border-b" style={{color:'var(--text-slate-900)',borderColor:'var(--border-slate-200)'}}>{pickup.duplicateCount} stops at this location</div>{[...(groupedPickupMarkers.get(locationKey)||[]),...(groupedDeliveryMarkers.get(locationKey)||[])].sort((a,b)=>(a.stop_order||0)-(b.stop_order||0)).map(m=>{const fin=FINISHED_STATUSES.includes(m.status);const ft=m.actual_delivery_time?format(new Date(m.actual_delivery_time),'HH:mm'):null;const nm=m.markerType==='pickup'?'Store Pickup':(m.patient?.full_name||'Patient');return(<div key={`ci-${m.id}`} className="text-xs py-1.5 border-b last:border-0 cursor-pointer hover:bg-slate-50 px-1 -mx-1 rounded space-y-0.5" style={{borderColor:'var(--border-slate-200)'}} onClick={()=>{document.querySelectorAll('.leaflet-popup').forEach(p=>p.remove());document.getElementById(`stop-card-${m.id}`)?.scrollIntoView({behavior:'smooth',block:'nearest',inline:'center'});}}><div className="flex items-center gap-1.5 font-medium" style={{color:'var(--text-slate-900)'}}><Truck className="w-3.5 h-3.5"/>{m.driver?.user_name||'Unknown'}</div><div className="flex items-center gap-1.5 text-[11px]" style={{color:'var(--text-slate-600)'}}><Home className="w-3.5 h-3.5"/>{m.store?.name||'Store'}</div>{fin&&ft?<div className="flex items-center justify-between text-[11px]"><span style={{color:'var(--text-slate-900)'}}>{nm}</span><span className="text-emerald-600">{ft}</span></div>:m.delivery_time_eta?<div className="flex items-center justify-between text-[11px]"><span style={{color:'var(--text-slate-900)'}}>{nm}</span><span style={{color:'var(--text-slate-600)'}}>ETA: {m.delivery_time_eta}</span></div>:<div className="text-[11px]" style={{color:'var(--text-slate-900)'}}>{nm}</div>}</div>);})}</div></Popup>):(<Popup autoPan={false} closeButton={false} offset={[0,-20]} className="custom-popup"><DeliveryPopup delivery={pickup} isPickup={true} stores={safeStores} patients={safePatients} users={safeUsers}/></Popup>))}
