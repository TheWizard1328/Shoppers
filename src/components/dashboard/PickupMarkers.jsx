    ...
    return [
      ...
      <Marker key={`pickup-${pickup.id}`} position={markerPosition} icon={icon} zIndexOffset={dynamicZIndex} draggable={!pickup.useSimpleCircle && !pickup.isOtherDriver && isFanned} eventHandlers={handlers} ref={(ref) => { if (ref) markerRefs.current[`pickup-${pickup.id}`] = ref; }}>
        ...
        {pickup.isOtherDriver && (
          <Popup autoPan={false} closeButton={false} offset={[0,-20]} className="custom-popup">
            <div className="min-w-[220px] max-w-[320px]">
              {isClustered && !isFanned && <div className="font-semibold text-sm pb-1 border-b mb-1" style={{color:'var(--text-slate-900)',borderColor:'var(--border-slate-200)'}}>{pickup.duplicateCount} stops at this location</div>}
              <MarkerInfoBalloon delivery={pickup} store={pickup.store} patient={null} driver={pickup.driver} isPickup={true} compact={isClustered && !isFanned} />
            </div>
          </Popup>
        )}
      </Marker>
