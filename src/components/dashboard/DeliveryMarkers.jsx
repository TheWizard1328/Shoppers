          ...
          isClustered && !isFanned ? (
            <Popup autoPan={false} closeButton={false} offset={[0, -20]} className="custom-popup">
              <div className="min-w-[220px] max-w-[320px]">
                <div className="font-semibold text-sm pb-1 mb-2 border-b" style={{ color: 'var(--text-slate-900)', borderColor: 'var(--border-slate-200)' }}>{delivery.duplicateCount} stops at this location</div>
                {(() => {
                  const lk = locationKey;
                  const all = [...(groupedPickupMarkers.get(lk) || []), ...(groupedDeliveryMarkers.get(lk) || [])]
                    .sort((a, b) => (a.stop_order || 0) - (b.stop_order || 0));

                  // Group by driver, preserving stop order
                  const driverGroups = [];
                  const seenDriverIds = [];
                  all.forEach((m) => {
                    const driverId = m.driver?.id || m.driver_id || 'unknown';
                    if (!seenDriverIds.includes(driverId)) {
                      seenDriverIds.push(driverId);
                      driverGroups.push({ driverId, driver: m.driver, stops: [] });
                    }
