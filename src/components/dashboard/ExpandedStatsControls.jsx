    ...
    <>
      <div className="pt-1 pb-1 border-t flex items-center gap-2" style={{ borderColor: 'var(--border-slate-200)', opacity: isInactiveDriver ? 0.5 : 1, pointerEvents: isInactiveDriver ? 'none' : 'auto' }}>
        <Select value={selectedDriverId} onValueChange={handleDriverChange} disabled={isDriverDropdownDisabled || isInactiveDriver}>
          <SelectTrigger className="whitespace-nowrap border-input bg-transparent shadow-sm data-[placeholder]:text-muted-foreground flex h-10 items-center justify-between rounded-md border px-3 py-2 text-sm ring-offset-background placeholder:text-muted-foreground focus:outline-none focus:ring-1 focus:ring-ring disabled:cursor-not-allowed disabled:opacity-50 [&>span]:line-clamp-1" style={{ pointerEvents: 'auto', touchAction: 'manipulation', background: 'var(--bg-white)', borderColor: 'var(--border-slate-300)', color: 'var(--text-slate-900)', minWidth: '120px', maxWidth: '160px', width: '100%' }}>
