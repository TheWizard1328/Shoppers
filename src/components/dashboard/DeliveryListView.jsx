...
const DeliveryRow = memo(({ 
  ...
  return (
    isMobile ? (
      ...
      <div
        onClick={() => onSelect(delivery.id)}
        className={`grid grid-cols-[auto_auto_auto_minmax(300px,1fr)_minmax(160px,1fr)_auto_auto_auto_auto_auto] gap-3 px-4 py-3 border-b cursor-pointer transition-colors ${
          isNextDelivery ? 'bg-blue-50 hover:bg-blue-100' : 'hover:bg-slate-50'
        } ${isSelected ? 'bg-slate-100' : ''}`}
        style={{ borderColor: 'var(--border-slate-200)' }}
      >
    ...
    <>
      <div className="h-full flex flex-col relative" style={{ background: 'var(--bg-white)' }}>
        ...
        <div className="flex-shrink-0 border-b sticky top-0 z-10" style={{ background: 'var(--bg-slate-50)', borderColor: 'var(--border-slate-200)' }}>
          {!isMobile && (
            <div className="grid grid-cols-[auto_auto_auto_minmax(300px,1fr)_minmax(160px,1fr)_auto_auto_auto_auto_auto] gap-3 px-4 py-3 text-sm font-semibold" style={{ color: 'var(--text-slate-700)' }}>
