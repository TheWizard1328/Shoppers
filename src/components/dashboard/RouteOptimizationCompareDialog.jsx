    ...
    <>
      ...
      >
        ...
        <div className="overflow-y-auto bg-white dark:bg-slate-950" style={{ maxHeight: isMobile ? "55vh" : "60vh" }}>
          {sorted.length === 0 ? (
            <div className="text-center text-sm text-slate-400 dark:text-slate-400 py-10">No stops to compare</div>
          ) : (
            sorted.map((row, idx) => {
              ...
              return (
                ...
                >
                  {/* Old stop order */}
                  <div className="text-center font-mono text-slate-400 dark:text-slate-400">
                    {row.oldStopOrder ?? "—"}
                  </div>
                  ...
                  <div className="text-center font-mono font-semibold text-emerald-700 dark:text-emerald-400">
                    {pending ? <span className="text-slate-300 dark:text-slate-400">…</span> : (row.newEta || "—")}
                  </div>

                  {/* New stop order */}
                  <div className="text-center font-mono font-bold text-emerald-700 dark:text-emerald-400">
                    {pending ? <span className="text-slate-300 dark:text-slate-400">…</span> : (row.newStopOrder ?? "—")}
                  </div>
                </div>
              );
            })
