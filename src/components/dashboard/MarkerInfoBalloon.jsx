...
export default function MarkerInfoBalloon({
  ...
  // Resolve ISP/ISD location for display
  ...
  const isISP = isISPOrISD && String(delivery?.delivery_id || '').toUpperCase().startsWith('ISP-');
  const isISD = isISPOrISD && String(delivery?.delivery_id || '').toUpperCase().startsWith('ISD-');
  ...
  const displayStoreName = isISD
    ...
    : isISP
      ? (ispSourceStore?.name || ispLoc?.store_name || 'Inter-Store')
      : (store?.name || 'Store');
