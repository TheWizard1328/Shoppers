...
export default function RouteManagementContent({
  ...
  const handleBulkEditApply = useCallback((values) => {
    ...
    if (values.delivery_date) updates.delivery_date = values.delivery_date;
    if (values.delivery_time_start) updates.delivery_time_start = values.delivery_time_start;
    if (values.delivery_time_end) updates.delivery_time_end = values.delivery_time_end;
    if (values.driverChoice === "unassigned") {
      ...
      if (selectedDriver) {
        updates.driver_id = selectedDriver.id;
        updates.driver_name = getDriverNameForStorage(selectedDriver);
      }
    }

    if (!selectedBulkDeliveryIds.length || Object.keys(updates).length === 0) {
    ...
    return Promise.all(
      selectedBulkDeliveryIds.map((deliveryId) => updateDeliveryLocal(deliveryId, updates, { isBatchOperation: true }))
    )
      .then(async () => {
  ...
  }, [bulkEditableDrivers, loadData, selectedBulkDeliveryIds]);
