    ...
    return (
      <Card className="bg-white dark:bg-slate-900 border-slate-200 dark:border-slate-700 shadow-sm">
        <CardContent className="p-8 text-center">
          ...
          <p className="text-slate-500 dark:text-slate-400">Select a delivery to view details</p>
        </CardContent>
      </Card>
    );
  }
      ...
      <Card className="border shadow-sm bg-card border-card">
        <CardHeader>
          <CardTitle className="flex items-center gap-2">
            ...
            <span className="text-body">Delivery Details</span>
          </CardTitle>
        </CardHeader>
          ...
          <div className="space-y-3">
            <div className="flex items-center gap-3 text-sm">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--bg-slate-100)' }}>
                <Calendar className="w-4 h-4 text-label" />
              </div>
              <div>
            ...
            {patient?.phone && (
              <div className="flex items-center gap-3 text-sm">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--bg-slate-100)' }}>
                  <Phone className="w-4 h-4 text-label" />
                </div>
                <div>
                  <p className="font-medium text-body">Patient Phone</p>
                  <span className="text-label">{formatPhoneNumber(patient?.phone)}</span>
                </div>
              </div>
            )}
            ...
            {store?.phone && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <div>
                  <p className="font-medium text-body">Store Phone</p>
                  <span className="text-label">{formatPhoneNumber(store?.phone)}</span>
                </div>
              </div>
            )}
            ...
            {delivery.driver_name && (
              <div className="flex items-center gap-3 text-sm">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--bg-slate-100)' }}>
                  <User className="w-4 h-4 text-label" />
                </div>
                <div>
                  <p className="font-medium text-body">Driver</p>
                  <p className="text-label">{delivery.driver_name}</p>
                </div>
              </div>
            )}
            ...
            {delivery.estimated_duration && (
              <div className="flex items-center gap-3 text-sm">
                <div className="w-8 h-8 rounded-lg flex items-center justify-center" style={{ background: 'var(--bg-slate-100)' }}>
                  <Clock className="w-4 h-4 text-label" />
                </div>
                <div>
                  <p className="font-medium text-body">Estimated Duration</p>
                  <p className="text-label">{delivery.estimated_duration} minutes</p>
                </div>
              </div>
            )}
            ...
            {delivery.actual_delivery_time && (
              <div className="flex items-center gap-3 text-sm">
                ...
                <div>
                  <p className="font-medium text-body">Delivered At</p>
                  <p className="text-label">
            ...
            {hasCOD && (
              <div className={`flex items-start gap-3 text-sm p-3 rounded-lg border ${
                ...
                <div className="flex-1">
                  <p className="font-medium mb-2 text-body">COD Payment</p>
                  
                  {/* Required Amount */}
                  <div className="flex items-center justify-between mb-2">
                    <span className="text-label">Required:</span>
          ...
          {(delivery.delivery_instructions || delivery.delivery_notes) && (
            <div className="pt-3 border-t space-y-3 border-card">
              {delivery.delivery_instructions && (
                <div className="flex items-start gap-3 text-sm">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mt-0.5" style={{ background: 'var(--bg-slate-100)' }}>
                    <AlertCircle className="w-4 h-4 text-label" />
                  </div>
                  <div>
                    <p className="font-medium mb-1 text-body">Delivery Instructions</p>
                    <p className="leading-relaxed whitespace-pre-wrap text-label">{delivery.delivery_instructions.replace(/ - /g, '\n')}</p>
                  </div>
                </div>
              )}
              ...
              {delivery.delivery_notes && (
                <div className="flex items-start gap-3 text-sm">
                  <div className="w-8 h-8 rounded-lg flex items-center justify-center mt-0.5" style={{ background: 'var(--bg-slate-100)' }}>
                    <FileText className="w-4 h-4 text-label" />
                  </div>
                  <div>
                    <p className="font-medium mb-1 text-body">Driver Notes</p>
                    <p className="leading-relaxed whitespace-pre-wrap text-label">{delivery.delivery_notes.replace(/ - /g, '\n')}</p>
                  </div>
                </div>
              )}
            </div>
        ...
        <Card className="border shadow-sm bg-card border-card">
          <CardHeader>
            <CardTitle className="flex items-center gap-2">
              ...
              <span className="text-body">Patient Information</span>
            </CardTitle>
          </CardHeader>
          <CardContent className="space-y-3">
            <div>
              <p className="text-sm font-medium text-body">{patient.full_name}</p>
              <p className="text-sm text-label">{patient.address}</p>
              {patient.phone && (
                <p className="text-sm mt-1 text-label">
