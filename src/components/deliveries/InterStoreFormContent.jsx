    ...
    <div className="flex flex-col gap-1 flex-1 min-w-0">
      ...
      <div className="relative flex items-center">
        <input
          ...
          disabled={disabled}
          className="w-full h-9 pl-3 pr-8 rounded-md border border-slate-300 dark:border-slate-600 bg-white dark:bg-slate-900 text-sm text-slate-900 dark:text-slate-100 focus:outline-none focus:ring-2 focus:ring-slate-400 disabled:opacity-50"
        ...
        <button
          type="button"
          onClick={() => onChange('')}
          className="absolute right-2 text-slate-400 hover:text-slate-600 dark:text-slate-400 dark:hover:text-slate-300">
  ...
  return (
    <div className="flex flex-col gap-2 flex-1 min-w-0">
      ...
      <div className="relative">
        <Search className="absolute left-2.5 top-1/2 -translate-y-1/2 w-3.5 h-3.5 text-slate-400 dark:text-slate-400 pointer-events-none" />
        <Input
          value={searchQuery}
      ...
      <div className="rounded-lg border overflow-hidden border-surface">
        <div ref={listRef} className="max-h-[220px] overflow-y-auto divide-y" style={{ borderColor: 'var(--border-slate-100)' }}>
          ...
          <div className="px-3 py-4 text-center text-xs text-slate-400 dark:text-slate-400">No locations found.</div>
          }
            ...
            return (
              <button
                ...
                <div className="flex items-center gap-1.5 flex-wrap">
                  <span className="font-semibold text-sm text-body">{loc.store_name}</span>
                  {loc.store_number && <span className="text-slate-400 dark:text-slate-400 text-xs">#{loc.store_number}</span>}
                  {userLat != null && userLng != null && loc.store_latitude != null && loc.store_longitude != null &&
                  <span className="text-[12px] text-slate-400 dark:text-slate-400 font-mono">
                ...
                <div className="flex items-center gap-1 mt-0.5">
                  <MapPin className="w-2.5 h-2.5 text-slate-400 dark:text-slate-400 shrink-0" />
                  <span className="text-slate-500 dark:text-slate-400 truncate text-xs">{loc.store_address}{loc.city ? `, ${loc.city}` : ''}</span>
                </div>
                {loc.store_phone &&
                <div className="flex items-center gap-1">
                    <Phone className="w-2.5 h-2.5 text-slate-400 dark:text-slate-400 shrink-0" />
                    <span className="text-[10px] text-slate-500 dark:text-slate-400">{formatPhoneNumber(loc.store_phone)}</span>
                  </div>
                }
              </button>);
    ...
    <div className="flex flex-col gap-3">
      ...
      <div className="flex items-center gap-2 px-3 py-2 rounded-lg border text-sm border-surface" style={{ background: 'var(--bg-slate-50)' }}>
          ...
          <div className="ml-auto flex items-center gap-1.5 shrink-0">
            {bothSelected && (
          distanceLoading ?
          <span className="text-[10px] text-slate-400 dark:text-slate-400 animate-pulse">Calculating...</span> :
          drivingDistance ?
          <span className="flex items-center gap-1 text-[10px] font-medium text-slate-600 dark:text-slate-400 bg-slate-100 dark:bg-slate-800 border border-slate-200 dark:border-slate-700 rounded px-1.5 py-0.5">
        ...
        <div className="flex flex-col gap-2 px-3 py-3 rounded-lg border border-surface" style={{ background: 'var(--bg-slate-50)' }}>
          <div className="flex flex-col gap-1 min-w-0">
            ...
            <Select value={formData.status || ''} onValueChange={(s) => setFormData((prev) => ({ ...prev, status: s }))} disabled={isSaving || statusLockedToInTransit}>
              <SelectTrigger className="h-9 text-sm w-full bg-white dark:bg-slate-900 border-slate-300 dark:border-slate-600">
