export const prepareDeliverySaveData = ({ formData, delivery, isCompletionStatus, completionTime, currentTravelMode = 'driving' }) => {
  ...
  if (formData.finished_leg_transport_mode || delivery?.finished_leg_transport_mode || delivery?.transport_mode || isNewDeliveryCreation || transitionsToPending) {
    // CRITICAL: prefer formData.transport_mode (user's current selection in the form) over saved delivery values
    const nextTravelMode = formData.transport_mode || formData.finished_leg_transport_mode || delivery?.transport_mode || delivery?.finished_leg_transport_mode || 'driving';
  ...
  if (isCompletionStatus) {
    ...
    // Support both HH:MM (standard) and full datetime (ISD/ISP form)
    const rawArrival = formData.arrival_time !== undefined ? formData.arrival_time : originalArrivalTime;
    const arrivalTimeValue = rawArrival && rawArrival.includes('T') ? rawArrival.substring(11, 16) : rawArrival;
