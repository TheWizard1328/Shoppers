...
function ProjectedDeliveryList({ deliveries, stopOrderMap }) {
  return (
    <div className="mt-3">
            ...
            {deliveries.map((delivery) => {
              ...
              return (
                <tr key={delivery.id} className="border-b border-slate-100 last:border-b-0">
                  ...
                  <td className="p-2 truncate">{delivery.patient_name}</td>
                  <td className="p-2 truncate text-right">{(delivery.distance_from_store ?? 0).toFixed(1)}km</td>
                </tr>
              );
            })}
          </tbody>
      ...
      <div className="w-full overflow-hidden shadow-lg border border-slate-200 rounded-lg bg-white">
        <div className="p-4 flex flex-col gap-2">
          <div className="flex items-start gap-3">
            <div
              className="flex-shrink-0 text-lg font-bold text-white w-8 h-8 flex items-center justify-center rounded-md"
            ...
            <div className="flex-shrink-0">
              <span
                ...
                style={{ backgroundColor: `${pickup.color || "#71717A"}20`, color: pickup.color || "#71717A" }}
              >
...
export function StatBox({ value, label, valueClass, onMeasure, fixedWidth }) {
  const ref = React.useRef(null);

  React.useEffect(() => {
    if (ref.current && onMeasure) {
      const width = ref.current.getBoundingClientRect().width;
      onMeasure(Math.ceil(width));
    }
