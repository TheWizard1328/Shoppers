...
export default function DeliveryForm({
  ...
  useEffect(() => {
    ...
    const handlePatientUpdated = (event) => {
      ...
      setFormData(prev => ({ ...prev, patient_name: updates.full_name || prev.patient_name, patient_phone: updates.phone || prev.patient_phone, unit_number: updates.unit_number || prev.unit_number, delivery_instructions: updates.notes || prev.delivery_instructions,
        // CRITICAL: Always sync time windows from the patient update so the form reflects the latest patient schedule
    ...
    const unsubscribeDelivery = base44.entities.Delivery.subscribe((event) => {
      ...
      // CRITICAL: Look up the patient's time windows so we never let a remote delivery
      // update overwrite the canonical patient-driven times on this device's form.
