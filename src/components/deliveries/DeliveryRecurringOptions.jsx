...
<div className="flex items-center space-x-2">
    ...
    <Label htmlFor={id} className={`text-sm font-medium leading-none ${disabled ? 'text-slate-400 dark:text-slate-400' : ''}`}>{label}</Label>
  </div>;
    ...
    <div className="flex-1 space-y-2 relative" id="recurring-section">
      ...
      <RadioGroup value={currentFrequency} onValueChange={handleFrequencyChange} disabled={!formData.recurring || isSaving} className="grid gap-2">
        ...
        <div key={value} className="flex items-center space-x-2">
            ...
            <Label htmlFor={value} className="text-sm font-medium peer-disabled:cursor-not-allowed peer-disabled:opacity-70 cursor-pointer">{label}</Label>
          </div>
        )}
      </RadioGroup>
    </div>);
