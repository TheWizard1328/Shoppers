...
import { isInterStoreDeliveryId } from "../utils/deliveryTypeUtils";
...
function DeliveryCard({
  ...
  const isInterStore = isInterStoreDeliveryId(delivery.delivery_id);
  const isBIK = delivery.is_cycling_marker === true;
  const isISP = isInterStore && String(delivery.delivery_id || '').toUpperCase().startsWith('ISP-');

  let displayName, displayAddress;

  if (isBIK) {
    displayName = delivery.delivery_notes || 'Cycling Marker';
    displayAddress = delivery.cycling_latitude != null
      ? `${Number(delivery.cycling_latitude).toFixed(5)}, ${Number(delivery.cycling_longitude).toFixed(5)}`
      : 'No coordinates';
  } else if (isInterStore) {
    const srcName = delivery._interstore_source_name || '';
    const dstName = delivery._interstore_dest_name || '';
    displayName = srcName && dstName ? `${srcName} → ${dstName}` : (isISP ? 'ISP Transfer' : 'ISD Transfer');
    displayAddress = store?.address || 'Inter-Store Transfer';
  } else if (isPickup) {
