...
export default function DeliveryStatusAndTiming({
  ...
  // For cycling markers: treat legacy en_route as in_transit
  ...
  // For cycling markers with active (in_transit) status: Status + Start Time + End Time on one row
  if (isCyclingMarker && isActive) {
    return (
      <>
        <style>{`
          .compact-time-input::-webkit-calendar-picker-indicator { display: none; -webkit-appearance: none; }
  ...
  // For cycling markers with completion status: show Status + Arrival + Completion on one row
  if (isCyclingMarker && isCompletion) {
    return (
      <>
        <style>{`
          .compact-time-input::-webkit-calendar-picker-indicator { display: none; -webkit-appearance: none; }
