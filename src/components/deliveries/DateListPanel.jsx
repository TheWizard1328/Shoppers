    ...
    <div className="flex flex-col h-full">
      <div className="px-2 py-2 flex-1 overflow-y-auto space-y-2">
        {itemsToRender.length === 0 ? (
          ...
          itemsToRender.map(({ date, dateStr: rawDateStr, total, completed, failed, returned, driversCount, canDelete }, index) => {
          // dateListWithStats uses 'date' as the key (no 'dateStr'), so normalise here
          const dateStr = rawDateStr || (typeof date === 'string' ? date : null);
          // Parse date string safely to avoid UTC→local timezone shift (off-by-1 day bug)
          ...
          return (
            ...
            >
              ...
              <div className="mt-1 flex items-center justify-between">
                ...
                <Badge variant="secondary" className="text-xs text-body-2" style={{ background: 'var(--bg-slate-200)' }}>
                  {total}
                </Badge>
              </div>
              ...
              <div className="flex items-center justify-between mb-1">
                ...
                <div className="flex items-center gap-1">
                  {isDispatcher && driversCount > 0 && (
                    <span className="text-xs flex items-center gap-1 text-soft">
