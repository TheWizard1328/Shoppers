...
export const GoogleAddressAutocomplete = forwardRef(function GoogleAddressAutocomplete({ 
  ...
  const handleSelectAddress = async (prediction) => {
    ...
    try {
      ...
      // Apply directional abbreviation (Northwest→NW etc.) since Google Places API (New)
      // returns longText for route components, which spells out full directional words.
      onChange(unabbreviateAddress(streetAddress));
