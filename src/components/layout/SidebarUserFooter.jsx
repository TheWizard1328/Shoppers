    ...
    return (
      <div className="border-t p-4 flex-shrink-0" style={{ borderColor: 'var(--border-slate-200)', background: 'var(--bg-white)' }}>
        <div className="space-y-2">
          <div className="text-sm text-slate-500 dark:text-slate-400 dark:text-slate-500 mb-2">Not logged in</div>
          <Button
            onClick={async () => {
              try {
      ...
      <div>
        ...
        <div className="pr-2 pl-2">
            ...
            onClick={() => setDriversExpanded((v) => !v)}>
              ...
              <span className="flex items-center gap-0 text-[10px] text-slate-400 dark:text-slate-500 dark:text-slate-400 font-medium">
              ...
              return (
                <div key={driver.user_id || driver.id} className="flex flex-col px-2 py-1.5 rounded-xl border cursor-pointer transition-all hover:shadow-sm active:scale-95" style={{ background: isAssigned ? 'linear-gradient(135deg, #eef2ff, #f5f3ff)' : 'var(--bg-slate-50)', borderColor: isAssigned ? '#c7d2fe' : 'var(--border-slate-200)' }} onClick={() => onOpenDriverChat?.(driver)} title={`Message ${driverName}`}>
                    <div className="flex items-center gap-1.5">
                      ...
                      {!isAssigned && <span className="text-[10px] px-1.5 py-0.5 rounded-full bg-slate-100 dark:bg-slate-800 text-slate-400 dark:text-slate-500 dark:text-slate-400 flex-shrink-0">Scheduled</span>}
                    </div>
        ...
        <div className="rounded-lg flex items-center gap-3 px-2">
          ...
          <div
            ...
            <LogOut className="w-4 h-4 text-slate-400 dark:text-slate-500 dark:text-slate-400 flex-shrink-0 hidden" />
            }
          </div>

          <div className="flex flex-col items-center">
            <button
              onClick={onOpenMessaging} className="px-2 py-0 rounded-lg hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 transition-colors relative"
              title="Messages">
              <MessageCircle className="w-5 h-5 text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300 dark:hover:text-slate-300" fill={unreadMessageCount > 0 ? '#10b981' : 'none'} />
              {unreadMessageCount > 0 &&
            ...
            <button
              ...
              <QrCode className="w-5 h-5 text-slate-500 dark:text-slate-400 dark:text-slate-500 hover:text-slate-700 dark:text-slate-300 dark:hover:text-slate-300" />
            </button>
          </div>
        </div>
