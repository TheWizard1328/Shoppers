...
const MobileBottomNav = React.forwardRef(function MobileBottomNav({ currentUser, currentPageName, onSidebarToggle, hasApkUpdate }, ref) {
  ...
  return (
    <nav
      ...
      className="relative z-[150] shrink-0 border-t bg-surface border-surface"
          ...
          {navItems.map((item) => {
            ...
            if (isMessagingItem) {
              return (
                ...
                >
                  <Icon className="w-5 h-5 mb-0.5 text-soft" />
                  <span className="text-sm font-medium truncate text-soft" style={{ maxWidth: '80px' }}>
                    {item.name}
                  </span>
                </button>
              );
