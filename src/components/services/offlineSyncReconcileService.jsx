      ...
      if (serverSample) {
        ...
        if (serverLatest > offlineLatest) {
          ...
          if (freshDeliveries && freshDeliveries.length > 0) {
            ...
            // setNextDeliveryFlag runs async — the API may still return false on the next stop.
            const offlineMap = new Map((offlineDeliveries || []).filter(d => d?.id).map(d => [d.id, d]));
            const toSave = freshDeliveries.map(d => {
              if (!d?.id) return d;
              const existing = offlineMap.get(d.id);
              if (existing?.isNextDelivery === true && !d.isNextDelivery) return { ...d, isNextDelivery: true };
              return d;
            });
            ...
            result.freshDeliveries = toSave; // use preserved version so callers get correct isNextDelivery
