        ...
        <div className="space-y-4">
          {payrollData.filter(data => data.grandTotal > 0).map((data, idx) => (
            <div key={data.driver.id} className={`p-3 rounded-lg ${idx % 2 === 0 ? 'bg-slate-50' : ''}`}>
              ...
              {/* Pay Rates Row */}
              <div className="inline-grid grid-cols-4 gap-x-4 text-xs mb-1.5" style={{ width: 'auto' }}>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">Rate:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{formatCurrency(data.payRate)}</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">KM:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{formatCurrency(data.extraKmRate, 3)}/km</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">OS:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{formatCurrency(data.oversizedRate)}</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">Failed:</span>
                  <span className="bg-red-100 text-red-700 px-1 py-0.5 rounded text-[11px] ml-1">{data.failedCount}</span>
                </div>
              </div>
              {/* Totals Row */}
              <div className="inline-grid grid-cols-4 gap-x-4 text-xs" style={{ width: 'auto' }}>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">Del:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{data.totalDeliveries} = {formatCurrency(data.totalBasePay)}</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">KM:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{data.totalExtraKm.toFixed(2)} = {formatCurrency(data.totalExtraKmPay)}</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">OS:</span>
                  <span className="bg-slate-200 text-slate-700 px-1 py-0.5 rounded text-[11px] ml-1">{data.oversizedCount} = {formatCurrency(data.totalOversizedPay)}</span>
                </div>
                <div className="flex items-center whitespace-nowrap">
                  <span className="text-slate-500">Returns:</span>
                  <span className="bg-orange-100 text-orange-700 px-1 py-0.5 rounded text-[11px] ml-1">{data.returnsCount}</span>
                </div>
              </div>
            </div>
          ))}
