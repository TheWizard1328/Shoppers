  ...
  return (
    <div data-notes-section="true" className="mt-2 flex flex-col gap-2 md:flex-row md:gap-3">
      ...
      <div className="md:flex-1">
          ...
          disabled={!canEditAdmin} className="w-full min-h-[75px] text-xs p-2 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 disabled:opacity-60"

          placeholder="Private notes (admins only)" />
        
          {savingAdmin && <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Saving...</div>}
        </div>
      }
      ...
      <div className="md:flex-1">
        ...
        <textarea
          ...
          disabled={!canEditDriver} className="w-full min-h-[75px] text-xs p-2 rounded border border-slate-200 dark:border-slate-700 bg-white dark:bg-slate-900 disabled:opacity-60"

          placeholder="Visible to driver + admins" />
        
        {savingDriver && <div className="text-[10px] text-slate-500 dark:text-slate-400 mt-1">Saving...</div>}
      </div>
    </div>);
