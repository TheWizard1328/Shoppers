...
/**
 * Generates a unique cycling-marker delivery ID (BIK)
 * Format: BIK-{5-char alphanumeric}  e.g. "BIK-Ab12X"
 * @param {string[]} existingIds - Array of existing delivery IDs to avoid duplicates
 * @returns {string}
 */
export const generateBikDeliveryId = (existingIds = []) => {
  const stripped = existingIds.map(id => id.replace(/^BIK-/, ''));
  const patientId = generatePatientId(stripped);
  return `BIK-${patientId}`;
};
