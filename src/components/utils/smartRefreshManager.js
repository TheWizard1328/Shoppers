...
class SmartRefreshManager {
  constructor() {
    ...
    // ADAPTIVE: Base intervals, adjusted by user activity monitor
    this.baseIntervals = {
    ...
    // CRITICAL: Intervals are now calculated dynamically based on activity
   ...
   * ADAPTIVE: Uses activity monitor to adjust intervals
   */
  shouldRefresh(type) {
     ...
     // ADAPTIVE: Calculate interval based on user activity
     const baseInterval = this.baseIntervals[type] || 30000;
     ...
     if (isIdle) {
       // Slow down by 3x when idle (except critical entities)
       interval = type === 'activeRoute' ? baseInterval : baseInterval * 3;
     } else if (activityLevel === 'active') {
       // Speed up by 0.5x when active (faster refreshes)
  ...
  /**
   * Fetches all AppUsers with deduplication
   */
  async fetchAllAppUsers() {
    try {
      // Use deduplicated coordinator
