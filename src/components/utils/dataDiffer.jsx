...
export const mergeEntityChanges = (currentState = [], { toUpdate, toAdd, toRemove }) => {
  ...
  if (toUpdate.length > 0) {
    ...
    merged = merged.map(item => {
      ...
      if (updated) {
        // CRITICAL: Shallow merge to preserve fields that may be missing in partial updates
        const merged = { ...item, ...updated };
        // Preserve specific fields that must never be overwritten with empty values
        if (item.puid && !updated.puid) merged.puid = item.puid;
        if (item.delivery_time_start && !updated.delivery_time_start) merged.delivery_time_start = item.delivery_time_start;
        if (item.delivery_time_end && !updated.delivery_time_end) merged.delivery_time_end = item.delivery_time_end;
        if (item.delivery_time_eta && !updated.delivery_time_eta) merged.delivery_time_eta = item.delivery_time_eta;
        return merged;
      }
