  ...
  const flushRealtimeBatch = useCallback(async () => {
    ...
    try {
      ...
      if (appUsersChanged) {
        ...
        // CRITICAL FIX: Use safeAppUserUpserts (IDB-merged full records) instead of
        // raw appUserUpserts (partial WS payloads). Raw upserts only carry changed fields
        // (e.g. current_latitude/longitude) and would REPLACE the full record in the
        // appUserById map, wiping location_tracking_enabled, city_id, driver_status,
        // app_roles, etc. This caused drivers to be invisible to each other because the
        // driver visibility check requires location_tracking_enabled to be true.
        safeAppUserUpserts.forEach((item) => {
          if (!item?.id) return;
    ...
    if (appUsersChanged && !nextAppUsers.length) {
      ...
      if (_fallbackAppUsers.length > 0) {
        ...
        // CRITICAL FIX: Use safeAppUserUpserts (IDB-merged full records) instead of
        // raw appUserUpserts (partial WS payloads) — same fix as the main path above.
        safeAppUserUpserts.forEach((item) => {
          if (!item?.id) return;
