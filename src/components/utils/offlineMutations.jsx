...
import {
  createGenericEntityLocal,
  updateGenericEntityLocal,
...
const getMeaningfulDeliveryUpdates = async (existingDelivery, updates = {}) => {
  ...
  return Object.entries(sanitizedUpdates).reduce((acc, [key, value]) => {
    // Force-update fields always pass through (explicit user intent)
    if (FORCE_DELIVERY_UPDATE_FIELDS.has(key)) {
      acc[key] = value;
      return acc;
    }
...
export const updateDeliveryLocal = async (deliveryId, updates, options = {}) => {
  ...
  try {
    ...
    if (newStatus && newStatus !== oldStatus) {
      ...
      const ACTIVE   = new Set(['in_transit', 'en_route']);
      const isPickup = !existingDelivery?.patient_id;
      const isAfterHoursPickup = isPickup && existingDelivery?.after_hours_pickup === true;
      const isDelivery = !!existingDelivery?.patient_id;

      // Rule 1: status → completed/failed/cancelled
      //   - deliveries: always fire
      //   - pickups: only if after_hours_pickup = true
      const rule1 = FINISHED.has(newStatus) && (isDelivery || isAfterHoursPickup);

      // Rule 2: status was completed/failed/cancelled → now in_transit/en_route
      //   - deliveries: always fire
      //   - pickups: only if after_hours_pickup = true
      const rule2 = FINISHED.has(oldStatus) && ACTIVE.has(newStatus) && (isDelivery || isAfterHoursPickup);

      // Rule 3: status was anything except staged → now pending
      //   - deliveries: always fire
      //   - pickups: only if after_hours_pickup = true
      const rule3 = newStatus === 'pending' && oldStatus !== 'staged' && (isDelivery || isAfterHoursPickup);

      if (rule1 || rule2 || rule3) {
        const driverId = existingDelivery?.driver_id || meaningfulUpdates?.driver_id;
        if (driverId) {
          import('@/api/base44Client').then(({ base44 }) => {
            base44.functions.invoke('calculateDriverPayroll', {
              data: { ...existingDelivery, ...meaningfulUpdates },
              event: { type: 'update' }
...
export const createCityLocal = (cityData) => createGenericEntityLocal({ entityName: 'City', entityData: cityData, mutationsPaused, notifyMutation });
export const updateCityLocal = (cityId, updates) => updateGenericEntityLocal({ entityName: 'City', recordId: cityId, updates, mutationsPaused, notifyMutation });
export const deleteCityLocal = (cityId) => deleteGenericEntityLocal({ entityName: 'City', recordId: cityId, mutationsPaused, notifyMutation });

export const createStoreLocal = (storeData) => createGenericEntityLocal({ entityName: 'Store', entityData: storeData, mutationsPaused, notifyMutation });
export const updateStoreLocal = (storeId, updates) => updateGenericEntityLocal({ entityName: 'Store', recordId: storeId, updates, mutationsPaused, notifyMutation });
export const deleteStoreLocal = (storeId) => deleteGenericEntityLocal({ entityName: 'Store', recordId: storeId, mutationsPaused, notifyMutation });

export const createCompanyLocal = (companyData) => createGenericEntityLocal({ entityName: 'Company', entityData: companyData, mutationsPaused, notifyMutation });
export const updateCompanyLocal = (companyId, updates) => updateGenericEntityLocal({ entityName: 'Company', recordId: companyId, updates, mutationsPaused, notifyMutation });
export const deleteCompanyLocal = (companyId) => deleteGenericEntityLocal({ entityName: 'Company', recordId: companyId, mutationsPaused, notifyMutation });