 ...
 *
 * gross_pay = raw base earnings (deliveries + extra km + oversized)
 * net_pay   = gross + tax - deductions + bonus + app fees
 */
...
export const calculateYtdPayroll = (ytdRecords, driverData, cities = [], appUser = null) => {
  // Sum all gross pay (raw earnings before deductions/bonus) from payroll records
  const ytdGrossPay = Math.round(ytdRecords.reduce((sum, r) => sum + (r.gross_pay || 0), 0) * 100) / 100;
  ...
  // Sum all tax amounts from payroll records, rounding to 2 decimals
  const ytdTaxAmount = Math.round(ytdRecords.reduce((sum, r) => sum + (r.tax_amount || 0), 0) * 100) / 100;

  // Net pay = gross + tax - deductions + bonus + app fees (stored correctly in net_pay)
