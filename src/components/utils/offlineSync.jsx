...
/**
 * Quick reconciliation: compares online vs offline DB for Deliveries (selected date) and AppUsers.
 * If the server has newer data (by count or updated_date), syncs offline DB and fires UI events.
 * Designed to be lightweight - only one API call per entity to check timestamp/count first.
 *
 * @param {string} selectedDateStr - The active/current date (YYYY-MM-DD)
 * @returns {Promise<{deliveriesUpdated: boolean, appUsersUpdated: boolean}>}
 */
export const quickReconcile = async (selectedDateStr) => {
  const result = { deliveriesUpdated: false, appUsersUpdated: false };
  ...
  try {
    // Get offline count and latest updated_date for this date
    const offlineDeliveries = await offlineDB.getByDate(offlineDB.STORES.DELIVERIES, selectedDateStr);
    const offlineCount = offlineDeliveries?.length || 0;
    const offlineLatestUpdated = offlineDeliveries?.reduce((max, d) => {
      const t = d.updated_date ? new Date(d.updated_date).getTime() : 0;
      return t > max ? t : max;
    }, 0);

    // Fetch just ONE record from server to get latest updated_date & total count cheaply
    const serverSample = await Delivery.filter({ delivery_date: selectedDateStr }, '-updated_date', 1);
    if (serverSample && serverSample.length > 0) {
      const serverLatestUpdated = new Date(serverSample[0].updated_date || 0).getTime();

      // Also get server count by fetching all IDs (lightweight field subset not possible, so fetch full but limit to IDs only comparison)
      // We use the timestamp check first - if newer record on server, always sync
      const needsSync = serverLatestUpdated > offlineLatestUpdated;

      if (needsSync) {
        console.log(`🔄 [QuickReconcile] Deliveries for ${selectedDateStr}: server has newer data. Syncing...`);
        const freshDeliveries = await Delivery.filter({ delivery_date: selectedDateStr }, '-updated_date', 5000);
        if (freshDeliveries && freshDeliveries.length > 0) {
          await offlineDB.bulkSave(offlineDB.STORES.DELIVERIES, freshDeliveries);
          invalidateEntityCache('Delivery');
          await offlineDB.updateSyncMetadata('Delivery', serverSample[0].updated_date, new Date().toISOString());
          result.deliveriesUpdated = true;
          result.freshDeliveries = freshDeliveries;
          console.log(`✅ [QuickReconcile] Synced ${freshDeliveries.length} deliveries for ${selectedDateStr}`);
        ...
        console.log(`✅ [QuickReconcile] Deliveries for ${selectedDateStr} are up-to-date (offline: ${offlineCount})`);
      }
    }
  } catch (e) {
    console.warn('⚠️ [QuickReconcile] Delivery check failed:', e.message);
  ...
  // --- AppUser reconciliation ---
  try {
    const offlineAppUsers = await offlineDB.getAll(offlineDB.STORES.APP_USERS);
    const offlineAppUserLatest = offlineAppUsers?.reduce((max, u) => {
      const t = u.updated_date ? new Date(u.updated_date).getTime() : 0;
      return t > max ? t : max;
    }, 0);

    const serverSampleUser = await AppUser.filter({}, '-updated_date', 1);
    if (serverSampleUser && serverSampleUser.length > 0) {
      const serverLatestUpdated = new Date(serverSampleUser[0].updated_date || 0).getTime();
      const needsSync = serverLatestUpdated > (offlineAppUserLatest || 0) || (offlineAppUsers?.length || 0) === 0;

      if (needsSync) {
        console.log(`🔄 [QuickReconcile] AppUsers: server has newer data or offline is empty. Syncing...`);
        const freshAppUsers = await fetchAppUsersDedup();
        if (freshAppUsers && freshAppUsers.length > 0) {
          // Deduplicate by user_id, prefer newest location_updated_at
          const userMap = new Map();
          freshAppUsers.forEach(au => {
            if (!au?.user_id) return;
            const existing = userMap.get(au.user_id);
            if (!existing) { userMap.set(au.user_id, au); return; }
            const newLoc = au.location_updated_at ? new Date(au.location_updated_at).getTime() : 0;
        ...
        console.log(`✅ [QuickReconcile] AppUsers are up-to-date (offline: ${offlineAppUsers?.length || 0})`);
      }
    }
  } catch (e) {
    console.warn('⚠️ [QuickReconcile] AppUser check failed:', e.message);
