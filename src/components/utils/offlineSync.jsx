...
export const performPrioritySyncBeforeRefresh = async (selectedDateStr, cityId = null, smartRefreshMgr = null, fetchAllDriversDeliveries = false) => {
  ...
  try {
    ...
    notifySyncStatus({ status: 'priority_sync_complete', deliveries: deliveries?.length || 0, patients: patients.length });
    
    return { success: true, deliveries: deliveries?.length || 0, patients: patients.length };
  } catch (error) {
