...
export function getNextActiveDelivery(driverDeliveries = [], currentDeliveryId = null, finishedStatuses = []) {
  return (driverDeliveries || [])
    .filter((item) =>
      ...
      !finishedStatuses.includes(item.status) &&
      (item.status !== 'pending' || item.is_cycling_marker === true)
    )
...
export async function collapseExpandedStopCardsForDriver(driverId) {
  ...
  // Scan the DOM for an expanded card belonging to this driver.
  // Only one card can be expanded at a time so we stop at the first hit.
  // If nothing is expanded, skip the event dispatch and double-RAF stall entirely.
  const expandedCard = Array.from(document.querySelectorAll('[data-driver-id]')).find((el) => {
    if (el.getAttribute('data-driver-id') !== driverId) return false;
    const surface = el.querySelector('.rounded-xl');
    return surface ? surface.offsetHeight > 72 : false;
  });
