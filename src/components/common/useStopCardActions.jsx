      ...
      Promise.resolve().then(async () => {
        try {
          ...
          backgroundTasks.push((async () => {
            await optimizeRouteAndApplyNextDelivery({ driverId: delivery.driver_id, deliveryDate: delivery.delivery_date, updateDeliveryLocal, updateDeliveriesLocally, forceRefreshDriverDeliveries, shouldRegeneratePolylines: false });
          })());
    ...
    const lockResult = await runWithDeliveryActionLock('retry_delivery', async () => {
      ...
      try {
        await withPausedDriverLocationPoller(async () => {
          ...
          try {
            await optimizeRouteAndApplyNextDelivery({ driverId: delivery.driver_id, deliveryDate: retryDate, updateDeliveryLocal, updateDeliveriesLocally, forceRefreshDriverDeliveries, shouldRegeneratePolylines: false, fallbackNextDeliveryId: newRetryDelivery?.id || newRetryDelivery?.data?.id || null });
          } catch {}
    ...
    const lockResult = await runWithDeliveryActionLock('restart_delivery', async () => {
      ...
      try {
        await withPausedDriverLocationPoller(async () => {
          ...
          try {
            const optimizationResult = await optimizeRouteAndApplyNextDelivery({ driverId: delivery.driver_id, deliveryDate: delivery.delivery_date, updateDeliveryLocal, updateDeliveriesLocally, forceRefreshDriverDeliveries, shouldRegeneratePolylines: false, fallbackNextDeliveryId: delivery.id });
  ...
  const handleCompleteAction = useCallback(async (e) => {
    ...
    const lockResult = await runWithDeliveryActionLock('complete_delivery', async () => {
      ...
      try {
        ...
        await setAndCenterNextDelivery({ driverDeliveries: routeDeliveries, targetDeliveryId: nextStop?.id || null, updateDeliveryLocal, updateDeliveriesLocally, driverId: delivery.driver_id, deliveryDate: delivery.delivery_date, skipBackgroundSync: true });
        if (!nextStop) {
        ...
        if (actedOnNextDelivery && shouldRecalculateCompletionEtas && nextStop) {
          backgroundTasks.push(base44.functions.invoke('calculateRealTimeETA', { driverId: delivery.driver_id, deliveryDate: delivery.delivery_date, currentLocalTime: String(completionUpdate.actual_delivery_time || '').match(/T(\d{2}:\d{2})/)?.[1] || getCurrentLocalTimeString() }).catch(() => {}));
        }
