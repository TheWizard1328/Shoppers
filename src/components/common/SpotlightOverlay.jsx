...
export default function SpotlightOverlay({ targetRef, text, visible, onClose, durationMs = 15000 }) {
  ...
  // Locate an optional anchor (e.g., Stats card) so overlay can render within it
  const [anchorEl, setAnchorEl] = useState(null);
  useEffect(() => {
    // Always call this hook; no-op when not visible
    if (!visible) { setAnchorEl(null); return; }
  ...
  const overlay = (
    <div {...ContainerProps}>
      ...
      {(() => {
        ...
        // If anchored, compute within anchor's box (local coords), else viewport
        const viewportPadding = 12;
        const contentLeft = anchorEl ? 12 : viewportPadding;
        const contentRight = anchorEl ? ((anchorRect?.width || 0) - 12) : (window.innerWidth - viewportPadding);
