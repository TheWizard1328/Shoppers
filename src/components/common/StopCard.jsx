...
export default function StopCard({ delivery, store, driver, patients = [], currentUser, isExpanded: externalIsExpanded, showDriverName = false, onStatusUpdate, onNotesUpdate, onEdit, onDelete, onRestart, allDeliveries = [], selectedDate, onEditPatient, drivers = [], onDriverChange, canEdit = false, getDriverColor, onClick, isSelected, isProjected = false, pendingPickups = [], onSelectionChange, selectedDeliveryIds = [], stopOrder = {}, onCODUpdate, stores = [], onCreateReturn, onStartDelivery, allStopsPending = false, onDriverStatusChange, appUsers = [], showDragHandle = false, dragHandleProps, compact = false, isRailCentered = true }) {
  ...
  const isReturnDelivery = useMemo(() => {if (!delivery || isPickup) return false;const patientName = (patient?.full_name || '').toUpperCase();const deliveryNotes = (delivery.delivery_notes || '').toUpperCase();const patientNotes = (patient?.notes || '').toUpperCase();const hasReturnMarker = patientName.includes('(RTN)') || deliveryNotes.includes('(RTN)') || patientNotes.includes('(RTN)');return hasReturnMarker && FINISHED_STATUSES.includes(delivery.status);}, [delivery, patient, isPickup]);
  ...
  const isActiveReturnDelivery = useMemo(() => {if (!delivery || isPickup) return false;const patientName = (patient?.full_name || '').toUpperCase();const deliveryNotes = (delivery.delivery_notes || '').toUpperCase();const patientNotes = (patient?.notes || '').toUpperCase();const hasReturnMarker = patientName.includes('(RTN)') || deliveryNotes.includes('(RTN)') || patientNotes.includes('(RTN)');return hasReturnMarker && !FINISHED_STATUSES.includes(delivery.status);}, [delivery, patient, isPickup]);
