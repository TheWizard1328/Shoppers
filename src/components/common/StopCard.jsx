...
export default function useStopCardActions(params) {
  ...
  const executeAcceptAllStops = useCallback(async () => {
    ...
    try {
      ...
      const now = new Date();
      // Detect retro date: if the selected delivery date is before today, use a time
      // derived from the latest actual_delivery_time on that route rather than now().
      // This prevents delivery_time_start from being stamped with today's real time on old routes.
      const isRetroDate = delivery.delivery_date < localDeviceTodayStr;
      let currentLocalTime;
      let deliveryTimeStart;
      if (isRetroDate) {
        // For retro routes, anchor to the latest finished stop's actual_delivery_time,
        // falling back to delivery_time_start of the pickup itself if nothing is finished yet.
        const routeForDate = allDeliveries.filter((d) => d && d.driver_id === delivery.driver_id && d.delivery_date === delivery.delivery_date);
        const finishedTimes = routeForDate
          .filter((d) => d?.actual_delivery_time && ['completed', 'failed', 'cancelled', 'returned'].includes(d.status))
          .map((d) => d.actual_delivery_time)
          .sort();
        const anchorTime = finishedTimes.at(-1) // latest finished time (HH:MM or ISO)
          || delivery.delivery_time_start       // pickup's own window
          || delivery.delivery_time_eta         // pickup's ETA
          || '08:00';
        // Normalise to HH:MM
        const timeMatch = String(anchorTime).match(/(\d{2}:\d{2})/);
        currentLocalTime = timeMatch ? timeMatch[1] : '08:00';
        const [anchorH, anchorM] = currentLocalTime.split(':').map(Number);
        const startMins = anchorH * 60 + anchorM + 5;
        deliveryTimeStart = `${String(Math.floor(startMins / 60) % 24).padStart(2, '0')}:${String(startMins % 60).padStart(2, '0')}`;
      } else {
    ...
    const lockResult = await runWithDeliveryActionLock(START_ACTION_NAME, async () => {
      ...
      try {
        ...
        try {
          ...
          // Step 2: Run optimizeRemainingStops FIRST so ETAs are recalculated before UI refresh
          let optimizedETAMap = new Map();
          try {
            ...
            optimizedRoute.forEach((stop) => {
              ...
              if (id && (stop.newETA || stop.eta)) {
                optimizedETAMap.set(id, {
            ...
            console.warn('âš ï¸ [Start] optimizeRemainingStops failed (non-fatal):', optErr?.message || optErr);
          }

          // Step 3: Fetch fresh deliveries from backend (post-optimization)
          const refreshedImmediately = await forceRefreshDriverDeliveries(delivery.driver_id, delivery.delivery_date);
          const refreshedListRaw = Array.isArray(refreshedImmediately)
            ? refreshedImmediately
            : Array.isArray(refreshedImmediately?.deliveries)
              ? refreshedImmediately.deliveries
              : null;

          // Merge optimized ETAs into the fresh list so UI shows correct times
          const refreshedListImmediate = Array.isArray(refreshedListRaw)
            ? refreshedListRaw.map((item) => {
                const etaOverride = optimizedETAMap.get(item?.id);
                return etaOverride ? { ...item, ...etaOverride } : item;
              })
            : null;
          ...
          if (Array.isArray(refreshedListImmediate) && refreshedListImmediate.length > 0) {
            ...
            // Broadcast to other devices (non-blocking)
            Promise.resolve().then(async () => {
              try {
            ...
            // Step 4: Update UI AFTER offline DB is persisted with correct ETAs
            window.dispatchEvent(new CustomEvent('deliveriesUpdated', { detail: { triggeredBy: 'startOptimized', driverId: delivery.driver_id, deliveryDate: delivery.delivery_date, alreadyOptimized: true, preserveLocalState: false, fullReplacement: true, freshDeliveries: refreshedListImmediate } }));
          } else {
          ...
          // Step 5: Trigger polyline redraw via FAB (non-blocking, polylines only â€” ETAs already done above)
          window.dispatchEvent(new CustomEvent('triggerManualRouteOptimization', {
            detail: {
      ...
      try {
        ...
        if (remainingEtaDeliveries.length > 0) {
          try {
            ...
            console.warn('âš ï¸ [Complete] Local ETA recalculation failed:', error?.message || error);
          }
        }
        if (!nextStop) {
      ...
      try {
        ...
        if (remainingEtaDeliveries.length > 0) {
          try {
            ...
            console.warn('âš ï¸ [Failure] Local ETA recalculation failed:', error?.message || error);
          }
        }
