...
import { useInterStoreLocation, parseInterStoreDeliveryId } from '../utils/interStoreDisplayName';
import { getDeliveryTypeFlags, resolveStopLocation } from '../utils/deliveryTypeUtils';
...
const _openClusterKeys = new Set();

function getClusterKey(driverId, deliveryDate, sortedDeliveryIds) {
  return `${driverId}:${deliveryDate}:${sortedDeliveryIds.join(',')}`;
}

export default function StopCard({ delivery, store, driver, patients = [], currentUser, showDriverName = false, onStatusUpdate, onNotesUpdate, onEdit, onDelete, onRestart, allDeliveries = [], selectedDate, onEditPatient, drivers = [], onDriverChange, canEdit = false, getDriverColor, onClick, isSelected, pendingPickups = [], onSelectionChange, selectedDeliveryIds = {}, onCODUpdate, stores = [], onCreateReturn, onStartDelivery, onDriverStatusChange, appUsers = [], showDragHandle = false, dragHandleProps, compact = false, isRailCentered = true, bulkSelectionEnabled = false, cardWidthOverride }) {
  const isNextDelivery = delivery?.isNextDelivery || false;
  const [squareAppId, setSquareAppId] = useState(_cachedSquareAppId);
  ...
  const [showMultiArrivalDialog, setShowMultiArrivalDialog] = useState(false);
  ...
  // ── Delivery type classification — declared here so all useEffects below can reference them ──
  const { isPickup, isInterStore: isISPorISD, isISP, isISD, isStorePickup } = useMemo(
    () => getDeliveryTypeFlags(delivery),
    [delivery?.patient_id, delivery?.delivery_id]
  );
  const interStoreLocation = useInterStoreLocation(delivery?.delivery_id);
  ...
  useEffect(() => {
    const refreshRangeCheck = (e) => {
      ...
      if (e?.type === 'driverLocationsUpdated') {
        const updated = e?.detail?.singleUpdate || null;
        const updatedList = e?.detail?.appUsers || null;
        const myId = currentUser?.id;
        if (myId) {
          const myRecord = updated?.user_id === myId
            ? updated
            : (updatedList || []).find((u) => u?.user_id === myId);
          if (myRecord?.current_latitude && myRecord?.current_longitude) {
            liveDriverLocationRef.current = {
              lat: Number(myRecord.current_latitude),
              lon: Number(myRecord.current_longitude)
            };
          }
        }
      }
      setRangeRefreshTick((prev) => prev + 1);
    };
  ...
  const ispSourceStore = useMemo(() => {
    if (!isISPorISD) return null;
    const parsed = parseInterStoreDeliveryId(delivery?.delivery_id);
    // "To" store = assignedStorePhone (parts[3]) for both ISP and ISD
    const toPhone = parsed?.assignedStorePhone;
    if (toPhone) {
      const byPhone = (stores || []).find((s) => s?.phone && String(s.phone).replace(/\D/g, '') === toPhone);
      if (byPhone) return byPhone;
    }
    // Fallback to delivery's own store_id
    if (delivery?.store_id) {
      return (stores || []).find((s) => s && s.id === delivery.store_id) || null;
    }
    return null;
  }, [isISPorISD, delivery?.delivery_id, delivery?.store_id, stores]);
  ...
  // isInterStorePickup is now derived from delivery_id prefix via getDeliveryTypeFlags
  ...
  // For ISP deliveries use the source store's color so badges reflect the originating store
  const storeColor = useMemo(() => {
    const effectiveStore = (isISPorISD && ispSourceStore) ? ispSourceStore : store;
    return effectiveStore ? getStoreColor(effectiveStore) : "#71717A";
  }, [isISPorISD, ispSourceStore, store]);
  ...
  // For ISP/ISD deliveries, use the InterStoreLocation GPS coords (the to/from location).
  // ISP: driver navigates TO the inter-store pickup location.
  // ISD: driver navigates TO the inter-store dropoff destination (same location entity, parts[2] phone).
  const rawStopLatitude = (isISPorISD && interStoreLocation?.store_latitude) ? interStoreLocation.store_latitude : (isPickup ? store?.latitude : patient?.latitude);
  const rawStopLongitude = (isISPorISD && interStoreLocation?.store_longitude) ? interStoreLocation.store_longitude : (isPickup ? store?.longitude : patient?.longitude);
  ...
  // Use live ref (updated on each driverLocationsUpdated event) for freshest coordinates,
  // falling back to currentDriverAppUser prop (which may be slightly stale between refreshes).
  const driverLat = Number(liveDriverLocationRef.current?.lat ?? currentDriverAppUser?.current_latitude ?? currentUser?.current_latitude);
  const driverLon = Number(liveDriverLocationRef.current?.lon ?? currentDriverAppUser?.current_longitude ?? currentUser?.current_longitude);
  ...
  // Show multi-delivery arrival dialog when driver arrives at a location with
  // multiple active deliveries at the same address.
  const coLocatedCount = useMemo(() => {
    if (!delivery?.patient_id || !patient?.latitude || !patient?.longitude) return 0;
    ...
    for (const d of allDeliveries) {
      ...
      if (!ACTIVE.has(d.status) || FINISHED.has(d.status)) continue;
  ...
  const hasArrived = !!(delivery?.arrival_time) || isWithinActiveStopRange;
  const isActivePatientStop = !isPickup && delivery?.patient_id && (delivery?.status === 'in_transit' || delivery?.status === 'en_route');

  // Build the cluster key for deduplication across cards at the same location
  ...
  // proximity threshold as the Navigate→GPS button swap).
  const hasPendingPickups = Array.isArray(pendingPickups) && pendingPickups.length > 0;
  const isAtPickupLocation = Boolean(
    isPickup &&
    hasPendingPickups &&
    (delivery?.status === 'en_route' || delivery?.status === 'in_transit') &&
    !delivery?.actual_delivery_time &&
    (delivery?.arrival_time || (isNextDelivery && isWithinActiveStopRange))
  );
        ...
        <CardContent className={`p-6 px-1 flex flex-col py-0 ${shouldAnchorExpandedCard ? 'max-h-full overflow-y-auto overscroll-contain' : ''}`}>
          ...
          <div className="flex flex-col">
              ...
              <div className="flex items-center gap-2 flex-shrink-0 ml-2">
                    {/* When driver has arrived at pickup: show Accept All, hide Call & Navigate */}
                    {isAtPickupLocation && canAccessAcceptButtons && isPickup && (
                      <button
                        type="button"
                        disabled={isAcceptingAll}
                        onClick={async (e) => { e.stopPropagation(); await handleAcceptAllStops(); }}
                        className="inline-flex items-center gap-2 h-9 px-3 rounded-md text-xs font-semibold !text-white bg-emerald-600 hover:bg-emerald-700 disabled:opacity-60 justify-center text-center leading-tight">
                        {isAcceptingAll ? 'Processing…' : acceptButtonText}
                      </button>
                    )}
          ...
          {/* Cold-chain temp logger — fires after complete/fail for fridge items */}
          {pendingCoolerLog && (
            <CoolerTempReader
              delivery={delivery}
              currentUser={currentUser}
          ...
          <FailureReasonDialog
            ...
            onConfirm={handleFailureConfirmWithBle}
            deliveryName={displayName}
            isPickup={isPickup}
            statusType={pendingFailureStatus} />

          <MultiDeliveryArrivalDialog
            open={showMultiArrivalDialog}
            onClose={() => {
              setShowMultiArrivalDialog(false);
              if (clusterKey) _openClusterKeys.delete(clusterKey);
            }}
            ...
            onFailDelivery={async (targetDelivery, reason) => {
              if (targetDelivery.id === delivery.id) {
                ...
                const now = new Date();
                const localTimeString = generateCompletionTimestamp(targetDelivery, allDeliveries, FINISHED_STATUSES);
                const existingNotes = targetDelivery.delivery_notes || '';
          ...
          <StopCardFooter
            ...
            handleStartAction={handleStartActionWithBle}
