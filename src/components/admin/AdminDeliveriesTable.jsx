...
export default function AdminDeliveriesTable(props) {
  ...
  const getSortIcon = (columnName) => {
    ...
    return <ArrowUpDown className="w-4 h-4 inline ml-1 text-slate-400 dark:text-slate-500 dark:text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity" />;
  };
    ...
    <Card className="flex-1 min-h-0 rounded-xl border bg-card text-card-foreground shadow">
      ...
      <CardContent className="px-6 py-1">
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <Select value={selectedDriver} onValueChange={onDriverChange} disabled={isLoadingData}><SelectTrigger className="w-40"><SelectValue placeholder="Select driver" /></SelectTrigger><SelectContent><SelectItem value="all">All Drivers</SelectItem>{drivers && drivers.length > 0 ? drivers.map((driver) => <SelectItem key={driver.id} value={driver.user_name || driver.full_name || ''}>{getDriverDisplayName(driver)}</SelectItem>) : <div className="p-2 text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500">No drivers available</div>}</SelectContent></Select>
        ...
        <div className="border rounded-md overflow-hidden" style={{ borderColor: 'var(--border-slate-200)' }}>
          <div className="overflow-x-auto" style={{ maxHeight: '600px' }}>
            <table className="w-full text-sm table-fixed">
              ...
              <tbody>
                {isLoadingData ? <tr><td colSpan={12} className="p-3 text-center text-slate-500 dark:text-slate-400 dark:text-slate-500"><Loader2 className="w-5 h-5 inline mr-2 animate-spin" />Loading deliveries...</td></tr> : displayDeliveries.length > 0 ? displayDeliveries.map((delivery) => {
