...
export default function AdminDeliveriesTable(props) {
  ...
  const getSortIcon = (columnName) => {
    ...
    return <ArrowUpDown className="w-4 h-4 inline ml-1 text-slate-400 dark:text-slate-400 opacity-0 group-hover:opacity-100 transition-opacity" />;
  };
    ...
    <Card className="flex-1 min-h-0 rounded-xl border bg-card text-card-foreground shadow">
      <CardHeader className="px-6 py-1 flex flex-col space-y-1.5">
        <CardTitle className="flex items-center justify-between">
          ...
          <div className="flex gap-2">
            ...
            {duplicateFilterMode && <Button variant="outline" size="sm" onClick={onClearDuplicateFilter} disabled={isLoadingData} className="bg-blue-50 dark:bg-blue-950 border-blue-300">Clear Filter</Button>}
      ...
      <CardContent className="px-6 py-1">
        <div className="flex flex-wrap gap-3 mb-4">
          ...
          <Select value={selectedDriver} onValueChange={onDriverChange} disabled={isLoadingData}><SelectTrigger className="w-40"><SelectValue placeholder="Select driver" /></SelectTrigger><SelectContent><SelectItem value="all">All Drivers</SelectItem>{drivers && drivers.length > 0 ? drivers.map((driver) => <SelectItem key={driver.id} value={driver.user_name || driver.full_name || ''}>{getDriverDisplayName(driver)}</SelectItem>) : <div className="p-2 text-xs text-slate-500 dark:text-slate-400">No drivers available</div>}</SelectContent></Select>
          <Select value={selectedCodFilter || 'all_deliveries'} onValueChange={onCodFilterChange} disabled={isLoadingData}><SelectTrigger className="w-40"><SelectValue placeholder="Select COD" /></SelectTrigger><SelectContent><SelectItem value="all_deliveries">All Deliveries</SelectItem><SelectItem value="all">All COD's</SelectItem><SelectItem value="cash">Cash</SelectItem><SelectItem value="debit">Debit</SelectItem><SelectItem value="credit">Credit</SelectItem><SelectItem value="cheque">Cheque</SelectItem></SelectContent></Select>
          ...
          <div className="flex items-center gap-2 px-3 py-2 rounded-lg border bg-card border-card"><Checkbox id="show-most-recent" checked={showMostRecentOnly} onCheckedChange={setShowMostRecentOnly} disabled={isCodFilterActive} /><label htmlFor="show-most-recent" className="text-sm font-medium cursor-pointer text-body">{isCodFilterActive ? 'Most Recent Date Only (off for COD filters)' : 'Most Recent Date Only'}</label></div>
        </div>
        <div className="border rounded-md overflow-hidden border-card">
          <div className="overflow-x-auto" style={{ maxHeight: '600px' }}>
            <table className="w-full text-sm table-fixed">
              ...
              <tbody>
                {isLoadingData ? <tr><td colSpan={12} className="p-3 text-center text-slate-500 dark:text-slate-400"><Loader2 className="w-5 h-5 inline mr-2 animate-spin" />Loading deliveries...</td></tr> : displayDeliveries.length > 0 ? displayDeliveries.map((delivery) => {
                  ...
                  return <tr key={delivery.id} className="border-b border-card"><td className="p-2"><Checkbox checked={selectedDeliveries.has(delivery.id)} onCheckedChange={(checked) => handleSelectDelivery(delivery.id, checked)} /></td><td className="p-2"><div className="flex flex-col"><span className="font-medium text-body">{dateTime.date}</span><span className="text-xs text-label">{dateTime.time}</span></div></td><td className="p-2 font-mono text-sm"><div className="flex flex-col"><span className="font-semibold">{delivery.stop_order ?? '-'}</span>{delivery.ampm_deliveries && <span className="text-xs text-slate-600 dark:text-slate-400">{delivery.ampm_deliveries}</span>}</div></td><td className="p-2 font-mono text-xs"><div className="flex flex-col">{delivery.stop_id && <span className="font-semibold">{delivery.stop_id}</span>}{info.patientPID && <span className="text-slate-600 dark:text-slate-400">{info.patientPID}</span>}{!delivery.stop_id && !info.patientPID && <span>-</span>}</div></td><td className="p-2 font-mono text-xs"><div className="flex flex-col"><span>{delivery.tracking_number || '-'}</span>{delivery.puid && <span className="text-slate-600 dark:text-slate-400 text-[10px]">{delivery.puid}</span>}</div></td><td className="p-2"><div className="flex flex-col"><span className="font-medium text-body">{info.name}</span><span className="text-xs text-label">{info.address}</span></div></td><td className="p-2">{editingDriverId === delivery.id ? <Select value={delivery.driver_id || ''} onValueChange={(newDriverId) => handleDriverChange(delivery, newDriverId)} onOpenChange={(open) => {if (!open) setEditingDriverId(null);}}><SelectTrigger className="h-7 w-full text-xs"><SelectValue /></SelectTrigger><SelectContent className="z-[9999]">{drivers.map((driver) => <SelectItem key={driver.id} value={driver.id}>{getDriverDisplayName(driver)}</SelectItem>)}</SelectContent></Select> : <div className="flex flex-col gap-1"><span className="cursor-pointer hover:bg-slate-100 dark:bg-slate-800 dark:hover:bg-slate-700 px-2 py-1 rounded transition-colors inline-block" onClick={() => setEditingDriverId(delivery.id)}>{driverName}</span>{delivery.isNextDelivery && <Badge className="bg-green-100 !text-green-800 border-green-300 w-fit">Next</Badge>}</div>}</td><td className="p-2"><div className="flex flex-col text-sm"><span className="font-mono" style={{ color: 'var(--text-slate-900 dark:text-slate-100)' }}>{Number(delivery.cod_total_amount_required || 0) > 0 ? `$${Number(delivery.cod_total_amount_required || 0).toFixed(2)}` : '-'}</span><span className="text-xs" style={{ color: 'var(--text-slate-600 dark:text-slate-400)' }}>{collectionType}</span></div></td><td className="p-2"><div className="flex flex-col text-sm font-mono" style={{ color: 'var(--text-slate-900 dark:text-slate-100)' }}><span>{delivery.travel_dist ? `${delivery.travel_dist.toFixed(2)}k` : '-'}</span>{(() => {const patient = (patients || []).find((p) => p.id === delivery.patient_id);const patientDist = patient?.distance_from_store;return patientDist ? <span className="text-xs text-slate-600 dark:text-slate-400 dark:text-slate-500">{patientDist.toFixed(2)}k</span> : null;})()}</div></td><td className="p-2">{getStatusBadge(delivery)}</td><td className="p-2"><DeliveryRouteDataCell delivery={delivery} /></td><td className="p-2 text-right"><div className="flex justify-end gap-2"><Button variant="ghost" size="icon" className="h-8 w-8" onClick={() => onEdit(delivery)}><Edit className="w-4 h-4" /></Button><Button variant="ghost" size="icon" className="h-8 w-8 text-red-600 hover:text-red-700 hover:bg-red-50 dark:bg-red-950 dark:hover:bg-red-950" onClick={() => onDelete(delivery)}><Trash2 className="w-4 h-4" /></Button></div></td></tr>;
                }) : <tr><td colSpan={12} className="p-3 text-center text-slate-500 dark:text-slate-400">No deliveries found.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
