...
export default function MessageRulesManager() {
  ...
  if (isLoading) {
    return <div className="flex items-center gap-2 p-6 text-slate-500 dark:text-slate-400"><Loader2 className="w-4 h-4 animate-spin" />Loading notification rules...</div>;
  }
      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
        ...
        <div>
          <div className="mb-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Edit the template and channels for each notification type.</p>
          </div>
        ...
        <div>
          <div className="mb-3">
            ...
            <p className="text-xs text-slate-500 dark:text-slate-400 mt-0.5">Control who receives each notification and set optional conditions.</p>
          </div>
      ...
      <Dialog open={showAddDialog} onOpenChange={(open) => { if (!open) { setShowAddDialog(false); setNewEventName(''); setNewLabel(''); } }}>
        <DialogContent className="max-w-sm">
          ...
          <div className="space-y-4 py-2">
            <div>
              <Label className="text-xs text-slate-600 dark:text-slate-400 mb-1 block">Event Key (internal identifier)</Label>
              <Input
                value={newEventName}
                ...
                className="text-sm"
              />
              <p className="text-xs text-slate-400 dark:text-slate-400 mt-1">Lowercase, underscores only. Must be unique.</p>
            </div>
            <div>
              <Label className="text-xs text-slate-600 dark:text-slate-400 mb-1 block">Display Label</Label>
              <Input
                value={newLabel}
