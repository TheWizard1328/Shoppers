...
function LogEntryCard({ log, matchingPatients = [], onAction, stores = [], disabled = false, isSelected = false, onSelect, patients = [] }) {
  ...
  return (
    <div
      className={`rounded-lg border shadow-sm overflow-hidden cursor-pointer transition-all ${isSelected ? 'border-blue-500 ring-2 ring-blue-200 bg-blue-50 dark:bg-blue-950' : 'bg-white dark:bg-slate-900 hover:border-slate-300 dark:border-slate-600'}`}
        ...
        <div className="flex items-start justify-between gap-2">
          <div className="min-w-0 flex-1">
            <div className="flex items-center gap-2 flex-wrap">
              ...
              <span className="font-semibold text-slate-900 dark:text-slate-100 text-sm">{log.patient_name || 'Unknown Patient'}</span>
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      <DialogContent className="sm:max-w-[95vw] w-[95vw] flex flex-col px-4 py-4" style={{ maxHeight: '92vh', height: '92vh' }}>
        ...
        <div className="flex flex-1 min-h-0 gap-4 py-3">
          ...
          {/* Right: Map */}
          <div className="flex-1 min-h-0 rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 relative">
            <PatientGPSMap
