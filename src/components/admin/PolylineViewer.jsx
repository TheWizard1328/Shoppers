...
export default function PolylineViewer({ users = [] }) {
  ...
  const handleResegmentConfirmed = async (selectedStopOrders) => {
    ...
    try {
      // The backend reads the master from the DB and uses pickBestMaster to
      // prefer the is_snapped master (and delete stale raw duplicates), so we
      // don't pass master_polyline here — the backend dedup is authoritative.
            ...
            >
              <div className="flex items-center justify-between gap-2 mb-1">
                ...
                {isBreadcrumb && item.saved_to_route && (
                  <Badge className="text-xs flex-shrink-0 ml-auto bg-green-600 text-slate-600 border-0">✓ Saved</Badge>
                )}
        ...
        <div onClick={handleClick}>
          <div className="flex items-center justify-between gap-2 mb-1">
            ...
            <div className="flex items-center gap-1 flex-shrink-0">
              ...
              {hasCrumb && (
                <Badge variant="secondary" className={`text-xs py-0 px-1.5 ${crumb.saved_to_route ? 'bg-green-600 text-slate-600' : crumb.imported_from_delivery ? 'bg-purple-600 text-slate-600' : ''}`}>
