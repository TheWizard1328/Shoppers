  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-slate-400 dark:text-slate-400 font-medium uppercase tracking-wide">{label}</p>
  ...
  return (
    <div className="absolute inset-y-0 right-0 w-full bg-white dark:bg-slate-900 flex flex-col z-10 shadow-xl">
      ...
      <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex-shrink-0">
        <div>
          <p className="text-xs text-slate-500 dark:text-slate-400 font-medium">{storeName || 'Pharmacy'}</p>
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{dateStr}</h3>
        </div>
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-50 dark:hover:bg-slate-800 transition-colors"
        >
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {delivery.signature_image_url && (
          <div className="mt-4">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 uppercase tracking-wide mb-2">Signature</p>
            <img
