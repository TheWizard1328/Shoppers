...
const STATUS_CONFIG = {
  completed: { label: 'Delivered', color: 'text-green-700', bg: 'bg-green-50 dark:bg-green-950', border: 'border-green-200', Icon: CheckCircle },
  failed: { label: 'Attempted — Not Delivered', color: 'text-red-700', bg: 'bg-red-50 dark:bg-red-950', border: 'border-red-200', Icon: XCircle },
  cancelled: { label: 'Cancelled', color: 'text-slate-600 dark:text-slate-400 dark:text-slate-500', bg: 'bg-slate-50 dark:bg-slate-800', border: 'border-slate-200 dark:border-slate-700', Icon: XCircle },
  in_transit: { label: 'In Transit', color: 'text-blue-700', bg: 'bg-blue-50 dark:bg-blue-950', border: 'border-blue-200', Icon: Truck },
  en_route: { label: 'En Route to You', color: 'text-blue-700', bg: 'bg-blue-50 dark:bg-blue-950', border: 'border-blue-200', Icon: Truck },
  pending: { label: 'Scheduled', color: 'text-amber-700', bg: 'bg-amber-50 dark:bg-amber-950', border: 'border-amber-200', Icon: Clock },
};
  ...
  return (
    <div className="flex items-start gap-3 py-2.5 border-b border-slate-100 last:border-0">
      ...
      <div className="flex-1 min-w-0">
        <p className="text-xs text-slate-400 dark:text-slate-500 dark:text-slate-400 font-medium uppercase tracking-wide">{label}</p>
...
export default function PatientDeliveryDetailPanel({ delivery, storeName, onClose }) {
  ...
  return (
    <div className="absolute inset-y-0 right-0 w-full bg-white dark:bg-slate-900 flex flex-col z-10 shadow-xl">
      {/* Header */}
      <div className="flex items-center justify-between p-4 border-b border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 flex-shrink-0">
        <div>
          <p className="text-xs text-slate-500 dark:text-slate-400 dark:text-slate-500 font-medium">{storeName || 'Pharmacy'}</p>
          <h3 className="text-base font-bold text-slate-900 dark:text-slate-100">{dateStr}</h3>
        </div>
        <button
          onClick={onClose}
          className="w-8 h-8 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-700 flex items-center justify-center hover:bg-slate-50 dark:bg-slate-800 dark:hover:bg-slate-800 transition-colors"
        >
          <X className="w-4 h-4 text-slate-600 dark:text-slate-400 dark:text-slate-500" />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 pb-4">
        ...
        {delivery.proof_photo_urls && delivery.proof_photo_urls.length > 0 && (
          <div className="mt-4">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-2 flex items-center gap-1.5">
        ...
        {delivery.signature_image_url && (
          <div className="mt-4">
            <p className="text-xs font-semibold text-slate-500 dark:text-slate-400 dark:text-slate-500 uppercase tracking-wide mb-2">Signature</p>
            <img
              src={delivery.signature_image_url}
              alt="Signature"
              className="w-full max-h-24 object-contain rounded-lg border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800 p-2"
            />
          </div>
        )}
      </div>
