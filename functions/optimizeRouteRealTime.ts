...
Deno.serve(async (req) => {
  ...
  try {
    ...
    // CRITICAL: stop_order offset must account for isNextDelivery if it exists
    const stopOrderOffset = isNextDeliveryStop ? 1 : 0;

    for (let i = 0; i < optimizedRoute.length; i++) {
      const stopIdx = optimizedRoute[i];
      const stop = stops[stopIdx];
      const newStopOrder = startingStopOrder + stopOrderOffset + i + 1;
