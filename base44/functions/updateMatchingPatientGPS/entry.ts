...
function extractStreetKey(address) {
  ...
  // Normalize compass directions BEFORE stripping them so "Northwest" matches "NW"
  value = value
...
Deno.serve(async (req) => {
  try {
    ...
    // ── PREVIEW ALL: batch preview for multiple log IDs in one call ────────
    if (action === 'previewAll') {
      const { logIds } = body;
      if (!logIds || !Array.isArray(logIds) || logIds.length === 0) {
        return Response.json({ error: 'Missing logIds array' }, { status: 400 });
      }

      // Fetch all log entries sequentially to avoid rate limits
      const logEntries = [];
      for (const lid of logIds) {
        const entry = await base44.asServiceRole.entities.PatientGPSLog.get(lid).catch((e) => {
      ...
      if (logEntries.length === 0) return Response.json({ results: {} });

      // Collect all unique store IDs needed
      const storeIdsNeeded = new Set();
      const patientIdsNeeded = new Set();
      for (const log of logEntries) {
        if (log.store_id) storeIdsNeeded.add(log.store_id);
        const srcId = log.source_patient_id || log.patient_id;
        if (srcId) patientIdsNeeded.add(srcId);
      }

      // Fetch all stores needed
      const storeMap = new Map();
      for (const sid of storeIdsNeeded) {
        const store = await base44.asServiceRole.entities.Store.get(sid).catch(() => null);
        if (store) storeMap.set(sid, store);
      }

      // Collect city IDs and build city->patients cache
      const cityIds = new Set();
      for (const store of storeMap.values()) {
        if (store.city_id) cityIds.add(store.city_id);
      }

      // Fetch all stores per city
      const cityStoreMap = new Map(); // cityId -> storeId[]
      const allStoreIds = new Set(storeMap.keys());
      for (const cityId of cityIds) {
        const cityStores = await base44.asServiceRole.entities.Store.filter({ city_id: cityId }, 'name', 500).catch(() => []);
        const ids = (cityStores || []).filter(Boolean).map((s) => { allStoreIds.add(s.id); storeMap.set(s.id, s); return s.id; });
        cityStoreMap.set(cityId, ids);
      }

      // Fetch patients per store (only once per store)
      const storePatientsMap = new Map(); // storeId -> patients[]
      for (const sid of allStoreIds) {
        const patients = await base44.asServiceRole.entities.Patient.filter({ store_id: sid }, '-updated_date', 1000).catch(() => []);
        storePatientsMap.set(sid, patients || []);
      }

      // Fetch source patients
      const sourcePatientMap = new Map();
      for (const pid of patientIdsNeeded) {
        const patient = await base44.asServiceRole.entities.Patient.get(pid).catch(() => null);
        if (patient) sourcePatientMap.set(pid, patient);
      }

      // Now compute matches for each log entry using cached data
      const results = {};
      for (const log of logEntries) {
        const srcPatientId = log.source_patient_id || log.patient_id;
        const sourcePatient = sourcePatientMap.get(srcPatientId);
        if (!sourcePatient) { results[log.id] = []; continue; }

        const sourceStreetKey = extractStreetKey(sourcePatient.address);
        if (!sourceStreetKey) { results[log.id] = []; continue; }

        const newLat = toNumber(log.new_latitude);
        const newLon = toNumber(log.new_longitude);

        const sourceStore = storeMap.get(log.store_id || sourcePatient.store_id);
        const relevantStoreIds = sourceStore?.city_id
          ? (cityStoreMap.get(sourceStore.city_id) || [sourceStore?.id].filter(Boolean))
          : [sourceStore?.id].filter(Boolean);

        const allPatients = relevantStoreIds.flatMap((sid) => storePatientsMap.get(sid) || []).filter(Boolean);
