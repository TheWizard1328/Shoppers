...
const buildMonthlyDeliveryCounts = (deliveries = []) => {
  ...
  deliveries.forEach((delivery) => {
    ...
    if ((delivery.patient_id || isInterStoreDelivery(delivery)) && (delivery.status === 'completed' || delivery.status === 'failed')) {
...
function processAdminMetrics(deliveries, stores, appUsers, patients, year, appFeeRate) {
  ...
  const calculateExtraKm = (delivery) => {
    ...
    // ISD/ISP: use paid_km_override only (no patient record)
    // Regular patient deliveries: use paid_km_override if set, else patient.distance_from_store
    if (!delivery.patient_id && !isInterStoreDelivery(delivery)) return 0;
    ...
    const patientDistance = delivery.patient_id ? Number(patientMap.get(delivery.patient_id)?.distance_from_store || 0) : 0;
