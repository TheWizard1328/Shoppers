...
Deno.serve(async (req) => {
  try {
    ...
    const countablePatientDeliveries = periodDeliveries.filter(d =>
      d.patient_id && !d.no_charge &&
      ((d.status === 'completed' || d.status === 'failed') ||
       (d.status === 'cancelled' && isPatientReturn(d)))
    );

    // After-hours pickups: no patient_id, after_hours_pickup=true, completed or cancelled
    const countableAfterHoursPickups = periodDeliveries.filter(d =>
      !d.patient_id && d.after_hours_pickup === true && !d.no_charge &&
