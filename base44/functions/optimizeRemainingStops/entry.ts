  ...
  try {
    ...
    if (preserveExistingOrder) {
      ...
      console.log('âœ… [optimizeRemainingStops] Preserving existing order and refreshing ETAs only');
       ...
       // -------------------------------------------------------------------
       // ONE-CALL STRATEGY:
       // Call 1 (Sequence): findsequence2 â€” lockedNextStop/origin â†’ [stops] â†’ home
       //   Returns optimized order + crow-flies ETAs. skipRoutingApi=true (no polylines here).
       // Polylines are generated by purgeAndRegeneratePolylines in a single HERE Router call
       //   that starts from trueOriginCoords (passed in the response), covering ALL legs including
       //   the first leg (origin â†’ stop1). This eliminates the former "Call 2" entirely.
       // -------------------------------------------------------------------
       ...
       console.log(`ðŸ  [optimizeRemainingStops] trueOrigin=(${trueOrigin.lat},${trueOrigin.lng}) | dest=${resolvedHomePosition ? 'HOME' : 'LAST_STOP'} | lockedNextStop=${lockedNextStop?.delivery?.id || 'none'}`);
      ...
      // â”€â”€ First leg (trueOrigin â†’ lockedNextStop): use cache only, no API call â”€â”€â”€â”€â”€â”€â”€â”€â”€â”€
      // Polylines for ALL legs (including this first leg) are generated in a single HERE
      // Router call inside purgeAndRegeneratePolylines, using currentPosition=trueOriginCoords.
      // This reduces the total HERE call count for the full optimize+polyline cycle to 1.
      ...
      if (lockedNextStop) {
        // Cache check: reuse existing first-leg data for ETA purposes if origin hasn't changed.
        // NOTE: We no longer make a separate HERE call here. purgeAndRegeneratePolylines will
        // call HERE Router once for all legs starting from trueOriginCoords.
        ...
        if (originUnchanged && hasCachedData) {
          // Use cached ETA values (polyline will still be refreshed by purgeAndRegeneratePolylines)
          firstLegDistKm = cachedDistKm;
          firstLegDurMin = cachedDurMin;
          console.log(`â™»ï¸ [optimizeRemainingStops] First-leg ETA cache HIT â€” dist=${firstLegDistKm}km, dur=${firstLegDurMin}min (polyline will be refreshed by purgeAndRegeneratePolylines)`);
        } else {
          console.log(`ðŸ“ [optimizeRemainingStops] First-leg ETA using crow-flies fallback (${firstLegDistKm.toFixed(2)}km) â€” real polyline delegated to purgeAndRegeneratePolylines`);
        }
      }
