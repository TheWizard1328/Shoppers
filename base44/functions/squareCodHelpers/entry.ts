...
export function hasCollectedCardPayment(delivery) {
  return (Array.isArray(delivery?.cod_payments) ? delivery.cod_payments : []).some((p) => ['Debit', 'Credit', 'Cheque', 'Check', 'debit', 'credit', 'cheque', 'check', 'card', 'Card'].includes(p?.type) && Number(p?.amount || 0) > 0);
}
