...
async function updateCatalogItem({catalogObjectId,catalogVersion,itemName,amountCents,locationId,deliveryId,patientName,accessToken}) {
  ...
  const updatedVariations=evs.length>0
    ?evs.map((v)=>({type:'ITEM_VARIATION',id:v.id,version:v.version,present_at_all_locations:false,present_at_location_ids:[locationId],item_variation_data:{...v.item_variation_data,name:'Default',pricing_type:'FIXED_PRICING',price_money:{amount:amountCents,currency:'CAD'}}}))
...
async function handleFetchPayments(base44, payload) {
  ...
  const storeByLocationId=new Map((stores||[]).map((s)=>{const c=activeConfigById.get(s?.square_location_config_id);return c?.square_location_id?[c.square_location_id,s]:null;}).filter(Boolean));
...
async function handleGetCodData(base44, payload={}) {
  ...
  const storeByLocationId=new Map(safeStores.map((s)=>{const c=activeConfigById.get(s?.square_location_config_id);return c?.square_location_id?[c.square_location_id,s]:null;}).filter(Boolean));
  const locationIds=Array.from(new Set(safeStores.map((s)=>activeConfigById.get(s?.square_location_config_id)?.square_location_id).filter(Boolean)));
