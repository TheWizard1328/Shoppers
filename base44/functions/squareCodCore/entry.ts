...
async function handleSyncCatalogItems(base44, payload={}) {
  ...
  for(const t of recentSquareTx){const ac=t?.amount_cents??Math.round(Number(t?.amount||0)*100);if(t?.delivery_id){if(!txByDeliveryId.has(t.delivery_id))txByDeliveryId.set(t.delivery_id,[]);txByDeliveryId.get(t.delivery_id).push(t);}if(t?.status&&t.status!=='pending'){if(t?.square_catalog_object_id)settledCatIds.add(t.square_catalog_object_id);settledItemSigs.add(buildItemSignature(t?.item_name,ac));settledComparableSigs.add(buildComparableLocationSignature(t?.item_name,ac,t?.location_id));
  // Only add to settledDLASigs when the transaction date is close to the catalog item date.
  // A transaction from weeks/months ago with the same patient name + amount must NOT mark a
  // future/current catalog item as settled — that's the Angela Dottor false-match scenario.
  const txDateIso=(t?.raw_square_data?.payment_date||t?.created_date||'').slice(0,10);const itemDateIso=toIsoDate(t?.item_name);
  const datesClose=(d1,d2)=>{if(!d1||!d2)return true;const diffMs=Math.abs(new Date(d1+'T00:00:00').getTime()-new Date(d2+'T00:00:00').getTime());return diffMs<=MATCH_DATE_STRICT_DAYS*86400000;};
  if(datesClose(txDateIso,itemDateIso)){for(const sig of buildLocationDateAmountSignatureCandidates(t?.location_id,t?.item_name,ac))settledDLASigs.add(sig);}}}
...
async function handleReconcile(base44, payload) {
  ...
  for (const tx of noMatchTransactions) {
    ...
    // Parse the transaction date for proximity scoring
    const txDateIso = (tx.raw_square_data?.payment_date || tx.created_date || '').slice(0, 10);
