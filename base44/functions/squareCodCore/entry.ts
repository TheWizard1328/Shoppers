...
async function handleSyncCatalogItems(base44, payload={}) {
  ...
  // Only add to settledDLASigs when the transaction date is on or after the catalog item date
  // AND within the strict window. A transaction from before the delivery date, or from weeks ago,
  // must NOT mark a current catalog item as settled.
  ...
  if(txOnOrAfterAndClose(txDateIso,itemDateIso)){for(const sig of buildLocationDateAmountSignatureCandidates(t?.location_id,t?.item_name,ac))settledDLASigs.add(sig);}}}
...
async function handleReconcile(base44, payload) {
  ...
  for (const tx of noMatchTransactions) {
    ...
    for (const d of noMatchDeliveries) {
      ...
      // Hard rule: transaction date must be on or after the delivery date
