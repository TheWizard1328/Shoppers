...
const hasCollectedCardPayment = (d) => (Array.isArray(d?.cod_payments)?d.cod_payments:[]).some((p)=>['Debit','Credit','Cheque','Check','debit','credit','cheque','check','card','Card'].includes(p?.type)&&Number(p?.amount||0)>0);
...
Deno.serve(async (req) => {
  try {
    ...
    if(action==='createCodItem'){await requireUser(base44);return Response.json(await base44.functions.invoke('squareCreateCodItem',payload));}
    if(action==='deleteCodItem'){await requireUser(base44);return Response.json(await base44.functions.invoke('squareDeleteCodItem',payload));}
    if(action==='syncSquareCods'){await requireUser(base44);return Response.json(await base44.functions.invoke('syncSquareCods',payload));}
