...
const PAYMENT_TYPE_EXACT_CARD = new Set(['Debit', 'Credit', 'Cheque', 'Check', 'card', 'debit', 'credit', 'cheque', 'check', 'Card']);
...
// for skip purposes — those bypass Square via the card machine (Check is
// treated like a card payment). Cash completions still need a catalog item
// created for register reconciliation.
...
async function handleSyncCatalog(base44, payload={}) {
  ...
  const COLLECTED_PAYMENT_TYPES = new Set(['Debit', 'Credit', 'Cheque', 'Check', 'debit', 'credit', 'cheque', 'check', 'card']);
