// Redeployed on 2026-05-30 - Via Superagent The Boss
...
Deno.serve(async (req) => {
  try {
    ...
    // Skip the deviation guard if there's no existing polyline — a fresh route always needs one generated.
    if (!existingType1 && !body?.force && !allowDeviationCheck) {
      console.log('[regenerateType1Polyline] No existing type1 polyline — bypassing deviation_only_guard to generate fresh polyline');
      // Fall through to generation
    } else if (!body?.force && !allowDeviationCheck) {
    ...
    if (routeSections.length > 0) {
      ...
      // Stamp first_leg_origin on the first incomplete stop so cache invalidation works correctly
      if (incompleteStops[0]?.id) {
        await base44.asServiceRole.entities.Delivery.update(incompleteStops[0].id, {
